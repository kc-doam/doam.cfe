...
Функция СоздатьШаблонПоПроцессу(ШаблонСсылка = Неопределено) Экспорт
	...
	Если ШаблонСсылка = Неопределено ИЛИ ШаблонСсылка.Пустая() Тогда
		ИмяТипа = БизнесПроцессы[Метаданные().Имя].ТипШаблона();	
	...
	ШаблонОбъект.Ответственный = Пользователи.ТекущийПользователь();
	...
	ШаблонОбъект.Автор = Пользователи.ТекущийПользователь();
	...
	ШаблоныБизнесПроцессов.ЗаписатьДоступностьШаблона(ШаблонОбъект.Ссылка, Ложь, Ложь);
