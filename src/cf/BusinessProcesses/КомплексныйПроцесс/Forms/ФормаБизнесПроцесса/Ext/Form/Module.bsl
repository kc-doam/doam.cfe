...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	МассивШаблоныВЭтапах = Новый Массив;
	Для Каждого Этап Из Объект.Этапы Цикл
		МассивШаблоныВЭтапах.Добавить(Этап.ШаблонБизнесПроцесса);
	КонецЦикла;
	
	ПриЗакрытииСервер(Объект.Ссылка, Объект.Схема, МассивШаблоныВЭтапах);
...
Процедура ПриЗакрытииСервер(ОбъектСсылка, Схема, МассивШаблоныВЭтапах)
	
	МультипредметностьВызовСервера.ПроцессПриЗакрытииНаСервере(ОбъектСсылка);

	Если Не ЗначениеЗаполнено(ОбъектСсылка) И ЗначениеЗаполнено(Схема) Тогда
		УстановитьПривилегированныйРежим(Истина);
		Схема.ПолучитьОбъект().УстановитьПометкуУдаления(Истина);
		УстановитьПривилегированныйРежим(Ложь);
	Иначе
		// тут режим Таблица 
		
		Если Не ЗначениеЗаполнено(ОбъектСсылка) Тогда
			УстановитьПометкуУдаленияЭтапов(МассивШаблоныВЭтапах);
		КонецЕсли;	
...
&НаСервереБезКонтекста
Процедура УстановитьПометкуУдаленияЭтапов(МассивШаблоныВЭтапах)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Для Каждого ШаблонБизнесПроцесса Из МассивШаблоныВЭтапах Цикл
		Если ЗначениеЗаполнено(ШаблонБизнесПроцесса.ВладелецШаблона) Тогда
			ШаблонБизнесПроцесса.ПолучитьОбъект().УстановитьПометкуУдаления(Истина);
		КонецЕсли;
	КонецЦикла;
