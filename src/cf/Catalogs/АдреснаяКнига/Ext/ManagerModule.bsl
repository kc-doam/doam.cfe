...
Процедура ОбновитьСписокПодчиненныхОбъектов(
	...
	Если Не ЗначениеЗаполнено(ОбъектАдреснойКниги) Тогда
		
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Пожалуйста обновите данные адресной книги: они были повреждены!'"));
		Возврат;
	...
	ОтображатьВАдреснойКниге = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ОбъектАдреснойКниги, "ОтображатьВАдреснойКниге");
	...
	Для Каждого Подчиненный Из НовыеПодчиненныеОбъекты Цикл
		...
		НовыйПодчиненный.Родитель = ОбъектАдреснойКниги;
