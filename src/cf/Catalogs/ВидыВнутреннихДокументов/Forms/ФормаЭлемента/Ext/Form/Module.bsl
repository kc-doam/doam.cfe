...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ЯвляетсяЗаявкойНаОплату Тогда
		ОтборПлательщик = Новый Структура("Наименование", Справочники.НаименованияСторон.Получатель);
		НайденныеСтроки = ШаблонДокумента.Стороны.НайтиСтроки(ОтборПлательщик);
		Если НайденныеСтроки.Количество() Тогда
			Получатель = НайденныеСтроки[0].Сторона;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не МногоШаблонов 
		...
		Если Не ШаблонСсылка.ПроверитьЗаполнение() Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
...
Процедура ПолучательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ОтображатьСотрудников", Ложь);
	ПараметрыФормы.Вставить("ВыбиратьСотрудников", Ложь);
