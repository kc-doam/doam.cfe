...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ВидДокумента) Тогда 
		...
		РеквизитыВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидДокумента,
			"ОбязательноеУказаниеОтветственного, ВестиУчетСторон, ЯвляетсяЗаявкойНаОплату, ВариантПодписания");
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(РегистрационныйНомер) Тогда
		...
		ВестиУчетТоваровИУслуг = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидДокумента, "ВестиУчетТоваровИУслуг");
		ВестиУчетНДС = Делопроизводство.УчитыватьНДС(Организация);
		УстановитьПривилегированныйРежим(Ложь);
		
		Если ВестиУчетТоваровИУслуг И ВестиУчетНДС Тогда
			СообщенияНекорректныеСтавки = Новый Массив;
			Для Сч = 0 По Товары.Количество() - 1 Цикл
				
				ТекстСообщения = Делопроизводство.СтавкаНДСДействительнаВТаблицеТоваровИУслуг(Товары[Сч].СтавкаНДС,
					ДатаРегистрации, Сч);
				
				Если ЗначениеЗаполнено(ТекстСообщения) Тогда
					СообщенияНекорректныеСтавки.Добавить(ТекстСообщения);
				КонецЕсли;
				
			КонецЦикла;
			
			Если СообщенияНекорректныеСтавки.Количество() Тогда
				ТекстИсключения = СтрСоединить(СообщенияНекорректныеСтавки, Символы.ПС);
				ТекстИсключения = СтрШаблон(НСтр("ru = 'Ошибка регистрации/записи зарегистрированного документа:%1%2'"),
	...
	РеквизитыВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидДокумента, 
		"ВестиУчетПоОрганизациям, ВестиУчетСторон, ВариантПодписания, ВестиУчетТоваровИУслуг");
