...
Процедура ПолучательАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = ЗаявкиНаОплатуВызовСервера.ДанныеВыбораПолучателя(Текст, Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОригиналПолученТекстНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не Элемент.Доступность Или Элемент.ТолькоПросмотр Тогда
		ТекстСообщения = ТекстНедоступногоПоляОригинал();
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "ОригиналПолученТекст");
		Возврат;
	КонецЕсли;
...
Процедура СтороныПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекущиеДанные.Подписан И ЕстьЭДО И МожноКорректироватьПодписан Тогда
		ЭтоВходящийЭДО = (НаправлениеЭДО = ПредопределенноеЗначение("Перечисление.НаправленияЭДО.Входящий"));
		...
		ИначеЕсли ТекущиеДанные.НомерСтроки = 2 И ЭтоВходящийЭДО Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ТоварыСтавкаНДСНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ДелопроизводствоКлиент.ДанныеВыбораСтавкиНДС(Объект);
...
Процедура ТоварыСтавкаНДСАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ДелопроизводствоКлиент.ДанныеВыбораСтавкиНДС(Объект, Текст);
...
Процедура ОткрытьТекстовыйОбраз(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОбъектСсылка", ТекущиеДанные.Ссылка);
	ПараметрыФормы.Вставить("ИзКарточкиФайла", Ложь);
	ПараметрыФормы.Вставить("ИзвлеченТекст", Ложь);
	ПараметрыФормы.Вставить("Распознан", Истина);
...
Процедура ВыполнитьДействиеПоЭДОПослеПроверкиСертификатов(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ИмяКоманды = "ПринятьДокументЭДО" Тогда
		...
		Если ДоверенностиПодписейОтправителяКорректны() Тогда
			ПринятьВходящийПоЭДО(Результат);
		Иначе
			ПоказатьПредупреждение(, 
				НСтр("ru = 'Не найдена доверенность подписи. Пожалуйста проверьте доверенность контрагента'"));
		КонецЕсли;
...
Процедура НастроитьФормуПоСостояниюДела()
	...
	// Для уничтоженного документа недоступно добавление виз и резолюций.
	Если СтруктураСостоянияДела.Состояние = СостоянияМенеджер.Уничтожено Тогда
		Если Резолюции.Количество() = 0 Тогда
			Элементы.ДобавитьПервуюРезолюцию.Видимость = Ложь;
			Элементы.ПоказатьУдаленныеРезолюции.Видимость = Ложь;
		Иначе
			Элементы.СтраницаЕстьРезолюции.ТолькоПросмотр = Истина;
			Элементы.КПРезолюций.Доступность = Ложь;
		КонецЕсли;
		Если ВизыСогласования.Количество() = 0 Тогда
			Элементы.ДобавитьПервуюВизуСогласования.Видимость = Ложь;
			Элементы.ПоказатьУдаленныеВизыСогласования.Видимость = Ложь;
		Иначе
			Элементы.ГруппаЕстьВизы.ТолькоПросмотр = Истина;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Функция ТекстНедоступногоПоляОригинал()
	
	Если ТипЗнч(СтруктураСостоянияДела) = Тип("Структура") И ЗначениеЗаполнено(СтруктураСостоянияДела.Состояние) Тогда
		СостояниеТекст = "";
		Если СтруктураСостоянияДела.Состояние
			= ПредопределенноеЗначение("Перечисление.СостоянияДелХраненияДокументов.ПереданоВАрхив") Тогда
			СостояниеТекст = НСтр("ru = 'передан в архив'");
		Иначе
			СостояниеТекст = НСтр("ru = 'уничтожен'");
		КонецЕсли;
		
		Возврат СтрШаблон(НСтр("ru='Добавление оригинала невозможно, поскольку документ %1'"), СостояниеТекст);
	КонецЕсли;
	
	Возврат НСтр("ru='В текущем состоянии документа добавление оригиналов запрещено'");
...
Процедура УстановитьДоступностьКоманд(ТекущиеДанные)
	...
	Если ТекущиеДанные = Неопределено
		...
		УстановитьДоступностьКоманды(Элементы["Распознать"+Префикс], Ложь);
		...
		УстановитьДоступностьКоманды(Элементы["ФайлыОткрытьТекстовыйОбраз"], Ложь);
		УстановитьДоступностьКоманды(Элементы["КонтекстноеМенюФайлыОткрытьТекстовыйОбраз"], Ложь);
	...
	Иначе
		...
		УстановитьДоступностьКоманды(Элементы["Распознать"+Префикс],
			...
			И Редактирует.Пустая());
		УстановитьДоступностьКоманды(Элементы["ФайлыРаспознать"+Префикс],
			Элементы["Распознать"+Префикс].Доступность);
		УстановитьДоступностьКоманды(Элементы["КонтекстноеМенюФайлыРаспознать"+Префикс],
			Элементы["Распознать"+Префикс].Доступность);
		
		УстановитьДоступностьКоманды(Элементы["ФайлыОткрытьТекстовыйОбраз"],
			РасширениеПоддерживается
			И ТекущиеДанные.СтатусРаспознаванияТекста
				= ПредопределенноеЗначение("Перечисление.СтатусыРаспознаванияТекста.Распознано"));
		УстановитьДоступностьКоманды(Элементы["КонтекстноеМенюФайлыОткрытьТекстовыйОбраз"],
			Элементы["ФайлыОткрытьТекстовыйОбраз"].Доступность);
...
Процедура УстановитьДоступностьПоЭП()
	...
	Если ПодписанЭДО Тогда
		УстановитьТолькоПросмотр = Истина;
		ЭтоВходящийЭДО = (НаправлениеЭДО = Перечисления.НаправленияЭДО.Входящий);
		
		Если ЭтоВходящийЭДО И Не ДокументПодписанНами Тогда
			Версия = 5;
...
Процедура УдалитьСвязь(Параметры)
	...
	НастройкаСвязи = СвязиДокументов.ПолучитьНастройкуСвязи(Параметры.Документ, СвязанныйДокумент, Параметры.ТипСвязи);
...
Функция ПроверитьВозможностьРегистрации(ТекущийОбъект)
	...
	Если ВестиУчетТоваровИУслуг И УчитыватьНДС Тогда
		...
		Для Сч = 0 По ТекущийОбъект.Товары.Количество() - 1 Цикл
			...
			ТекстСообщения = Делопроизводство.СтавкаНДСДействительнаВТаблицеТоваровИУслуг(ТекущийОбъект.Товары[Сч].СтавкаНДС,
				ДатаРегистрацииПроверка, Сч);
...
Процедура УстановитьВидимостьПоЭДО()
	...
	ЕстьЭДО = ЕстьЭДО И ЗначениеЗаполнено(СостояниеПоЭДО);
...
Функция ОписаниеКомандЭДО()
	...
	ОписаниеКоманд.Вставить(Элементы.ПринятьДокументЭДО, КомандаПринять);
	...
	ОписаниеКоманд.Вставить(Элементы.КомандаПринятьЭДО, КомандаПринятьДополнительная);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаОтклонить = ШаблоныКоманд.ДействияЭДО.ОбработкаВходящих.ОтклонитьДокумент;
	ОписаниеКоманд.Вставить(Элементы.ОтклонитьДокументЭДО, КомандаОтклонить);
	...
	ОписаниеКоманд.Вставить(Элементы.КомандаОтклонитьЭДО, КомандаОтклонитьДополнительная);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаПодписать = ШаблоныКоманд.ДействияЭДО.ОбработкаИсходящих.ПодписатьДокумент;
	ОписаниеКоманд.Вставить(Элементы.ПодписатьДокументЭДО, КомандаПодписать);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаПодписатьИОтправить = ШаблоныКоманд.ДействияЭДО.ОбработкаИсходящих.ПодписатьИОтправитьДокумент;
	ОписаниеКоманд.Вставить(Элементы.ПодписатьИОтправитьЭДО, КомандаПодписатьИОтправить);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаОтправить = ШаблоныКоманд.ДействияЭДО.ОбработкаИсходящих.ОтправитьДокумент;
	ОписаниеКоманд.Вставить(Элементы.ОтправитьПоЭДО, КомандаОтправить);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаАннулировать = ШаблоныКоманд.ДействияЭДО.ПрочиеДействия.Аннулировать;
	ОписаниеКоманд.Вставить(Элементы.АннулироватьДокументЭДО, КомандаАннулировать);
	ОписаниеКоманд.Вставить(Элементы.КомандаАннулироватьЭДО, КомандаАннулировать);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаЗакрыть = ШаблоныКоманд.ДействияЭДО.ПрочиеДействия.ЗакрытьПринудительно;
	ОписаниеКоманд.Вставить(Элементы.ЗакрытьОбменПоЭДОПринудительно, КомандаЗакрыть);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаПоставитьВОчередьОтправки = ШаблоныКоманд.ДействияЭДО.ПрочиеДействия.ПоставитьВОчередьОтправки;
	ОписаниеКоманд.Вставить(Элементы.ПоставитьВОчередьЭДОНаОтправку, КомандаПоставитьВОчередьОтправки);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаСоздатьПакетЭДО = ШаблоныКоманд.РаботаСПакетом.СоздатьПакетЭДО;
	ОписаниеКоманд.Вставить(Элементы.СоздатьПакетЭДО, КомандаСоздатьПакетЭДО);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаДобавитьКПакету = ШаблоныКоманд.РаботаСПакетом.ДобавитьКПакету;
	ОписаниеКоманд.Вставить(Элементы.ДобавитьКПакетуЭДО, КомандаДобавитьКПакету);
	
	ШаблоныКоманд = ИнтерфейсЭДОДокументооборот.ШаблоныКомандЭДО();
	КомандаОткрытьПакет = ШаблоныКоманд.РаботаСПакетом.ОткрытьПакет;
	ОписаниеКоманд.Вставить(Элементы.ОткрытьПакетЭДО, КомандаОткрытьПакет);
...
Процедура ЕстьЭДО() Экспорт
	...
	ИнтерфейсЭДОДокументооборот.ЗаполнитьПараметрыФормыПоЭДО(ЭтотОбъект, Объект.Ссылка);
...
&НаСервере
Функция ДоверенностиПодписейОтправителяКорректны()
	
	ДокументЭДО = ОбменСКонтрагентамиДОСлужебный.ЭлектронныйДокументДокументаДО(Объект.Ссылка);
	
	Если ТипЗнч(ДокументЭДО) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящийЭДО") Тогда
	...
	Если Выборка.Следующий() Тогда
		Подписи = ЭлектронныеДокументыЭДО.УстановленныеПодписиСУчетомДоверенностей(Выборка.Сообщение);
	Иначе
		Возврат Истина;
	КонецЕсли;
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		Если Не ЗначениеЗаполнено(Подпись.СвойстваДоверенности) Тогда
			Возврат Ложь;
		КонецЕсли;
