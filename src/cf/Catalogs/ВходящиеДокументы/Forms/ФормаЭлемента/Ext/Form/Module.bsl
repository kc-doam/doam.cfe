...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Рабочие группы
	РаботаСРабочимиГруппами.ДокументПриСозданииНаСервере(ЭтотОбъект);
	...
	Делопроизводство.ПрочитатьЭтапы(ЭтотОбъект);
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСервисСПАРКРиски") Тогда 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("Регистрация") Тогда
		...
		Если НеобходимоПроверитьЭППередРегистрацией(ПараметрыЗаписи) Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	МЭДОПереопределяемый.СохранитьДанныеМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Входящее);
...
Процедура НастроитьФормуПоСостояниюДела()
	...
	// Для уничтоженного документа недоступно добавление резолюций.
...
Процедура ЗарегистрироватьНаСервере(ТекущийОбъект, ОтразитьВозвратВЖурнале, РегистрационныйНомер = "")
	...
	МЭДОПереопределяемый.ПриРегистрацииДокумента(ТекущийОбъект, Перечисления.НаправленияСообщенийМЭДО.Входящее);
...
Функция ДокументАктивен()
	...
	Если СтруктураСостоянияДела <> Неопределено
		И СтруктураСостоянияДела.Состояние
			<> ПредопределенноеЗначение("Перечисление.СостоянияДелХраненияДокументов.ПустаяСсылка") Тогда
...
Процедура ЗаполнитьЗакладкуМЭДОВФормеСервер()
	...
	МЭДОПереопределяемый.ЗаполнитьЗакладкуМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Входящее);
	// От заполнения полей могла измениться видимость:
	МЭДОПереопределяемый.ВидимостьЭлементовМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Входящее);
...
#Область ПроверкаЭППередРегистрацией

&НаКлиенте
Функция НеобходимоПроверитьЭППередРегистрацией(ПараметрыЗаписи)
	
	Если Не ИспользоватьЭП
		Или ЭлектронныеПодписи.ПолучитьЭлементы().Количество() = 0 Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	Если ПараметрыЗаписи.Свойство("ПровереныЭППередРегистрацией")
		И ПараметрыЗаписи.ПровереныЭППередРегистрацией = Истина Тогда
		
		Возврат Ложь;
	КонецЕсли;
	
	ОбработчикЗавершения = Новый ОписаниеОповещения("ОбработатьПроверкуЭППередРегистрацией",
...
&НаКлиенте
Процедура ОбработатьПроверкуЭППередРегистрацией(Результат, ПараметрыЗаписи) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Структура")
		Или Не Результат.Свойство("Успех")
		Или Результат.Успех <> Истина Тогда
		
		ПоказатьПредупреждение(, НСтр("ru = 'Регистрация невозможна, не удалось проверить электронные подписи документа'"));
		Возврат;
	КонецЕсли;
	...
	ПараметрыЗаписи.Вставить("ПровереныЭППередРегистрацией", Истина);
	Записать(ПараметрыЗаписи);
