...
Процедура ПередЗаписью(Отказ)
	...
	// Сделано через функцию БСП, т.к. иначе будет конфликт имен с ТЧ "Пользователи"
	//
	ОбщийМодульПользователи = ОбщегоНазначения.ОбщийМодуль("Пользователи");
	ЭтоПолноправный = ОбщийМодульПользователи.ЭтоПолноправныйПользователь();
	Если Не ЭтоПолноправный И Не ЗначениеЗаполнено(Родитель) Тогда
		
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Запрещено изменять корневую группу'"));
		Отказ = Истина;
		Возврат;
