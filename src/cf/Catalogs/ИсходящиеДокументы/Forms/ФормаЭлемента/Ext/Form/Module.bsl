...
//		Элементы.ГруппаСпискиФайлов.ТекущаяСтраница = Элементы.ГруппаСписок;
	...
	МЭДОПереопределяемый.СохранитьДанныеМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Исходящее);
...
&НаКлиенте
Процедура ПолучателиСпособОтправкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные.СпособОтправки = ПредопределенноеЗначение("Справочник.СпособыДоставки.МЭДО")
		И Объект.Ссылка.Пустая() Тогда 
		Если Не Записать() Тогда 
			Возврат;
		КонецЕсли;
...
Процедура СкопироватьПолучателяИзРеквизитовВСписок()
	...
	Объект.Получатели.Очистить();
	Объект.Получатели.Добавить();
	ЗаполнитьЗначенияСвойств(Объект.Получатели[0], ЭтотОбъект,
		"Получатель, ПолучательЮрЛицо, 
		|Адресат, ПолученОтвет, Отправлен, ДатаОтправки, 
		|СпособОтправки, ВходящаяДата, ВходящийНомер, ТребуетсяПодписьЭП");
...
Процедура НастроитьФормуПоСостояниюДела()
	...
	// Для уничтоженного документа недоступно добавление виз.
...
Процедура ЗарегистрироватьНаСервере(ТекущийОбъект, СообщениеОбОшибке, РегистрационныйНомер = "")
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.РегистрационныйНомер) Тогда // успешно сформирован номер
		ТекущийОбъект.Зарегистрировал = Пользователи.ТекущийПользователь();
	...
	МЭДОПереопределяемый.ПриРегистрацииДокумента(ТекущийОбъект, Перечисления.НаправленияСообщенийМЭДО.Исходящее);
...
Процедура ЗаполнитьЗакладкуМЭДОВФормеСервер()
	...
	МЭДОПереопределяемый.ЗаполнитьЗакладкуМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Исходящее);
...
Процедура ВидимостьЭлементовМЭДОВФормеСервер()
	...
	МЭДОПереопределяемый.ВидимостьЭлементовМЭДОВФорме(ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Исходящее);
...
Функция ГотовностьКОтправкеМЭДО_Сервер(Отказ)
	...
	Если ГотовКОтправкеМЭДО Тогда
		Если Не МЭДОПереопределяемый.ДокументГотовКОтправке(
			ЭтотОбъект, Перечисления.НаправленияСообщенийМЭДО.Исходящее, Элементы.ГруппаМЭДО.Видимость, Истина) Тогда
