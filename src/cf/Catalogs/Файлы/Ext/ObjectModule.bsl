...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		...
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Файлы.Ошибка записи файла при обновлении ИБ'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если Не ЭтоНовый() Тогда
		...
		// ИзмененаПометкаУдаления = Истина, но файл может быть один раз уже перезаписан, и файл второй раз
		// перезаписывается при перезаписи текущей версии файла, второй раз все проверки делать не надо,
		// плюс файл может быть служебный и при второй перезаписи в него не приходит доп. свойство
		// "УдалениеУлежебногоФайла". Поэтому нельзя ориентироваться на ИзмененаПометкаУдаления:
		Если ПометкаУдаления <> ЕстьПометкаУдаленияВИБ Тогда
			
			Если Не ДополнительныеСвойства.Свойство("УдалениеСлужебногоФайла")
				Или Не ДополнительныеСвойства.УдалениеСлужебногоФайла Тогда			
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("ЭтоНовый") И ДополнительныеСвойства.ЭтоНовый Тогда
		...
		ПротоколированиеРаботыПользователей.ЗаписатьСоздание(
			Ссылка, Истина);
	КонецЕсли;	
