...
Процедура СделатьКомандыНедоступными()
	...
	Элементы.ФормаОткрытьТекстовыйОбраз.Доступность = Ложь;
	Элементы.СписокКонтекстноеМенюОткрытьТекстовыйОбраз.Доступность = Ложь;
...
Процедура УстановитьДоступностьКоманд(РедактируетТекущийПользователь, Редактирует, ПодписанЭП, Зашифрован)
	...
	ТекстФайла = ТекстФайла(Элементы.Список.ТекущиеДанные.Ссылка);
	
	Элементы.ФормаРаспознать.Доступность =
		РасширениеПоддерживается
		И ДоступноРаспознаваниеПоЗапросу
		И Редактирует.Пустая()
		И Элементы.Список.ТекущиеДанные.СтатусРаспознаванияТекста
	...
	Элементы.ФормаОткрытьТекстовыйОбраз.Доступность = (РасширениеПоддерживается И ЗначениеЗаполнено(ТекстФайла));
	Элементы.СписокКонтекстноеМенюОткрытьТекстовыйОбраз.Доступность = Элементы.ФормаОткрытьТекстовыйОбраз.Доступность;
...
&НаСервереБезКонтекста
Функция ТекстФайла(Файл)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Файл, "ТекстХранилище").Получить();
