	...
	Если Глубина <= 10 Тогда
		...
		Для Каждого Вложение Из ВложенныеПисьма Цикл
			...
			ПараметрыОтправкиВложенногоПисьма 
				= ПолучитьСтруктуруПараметровОтправкиПисьма(
					ВложенноеПисьмоОбъект,,ЭтоВложенноеПисьмо, Глубина);
...
Функция ПолучитьТекстПриглашения(ФайлСсылка) Экспорт
	...
	ТекстСообщения = Документы.ВходящееПисьмо.СформироватьТекстПриглашения(ТекстФайла, Тема);
