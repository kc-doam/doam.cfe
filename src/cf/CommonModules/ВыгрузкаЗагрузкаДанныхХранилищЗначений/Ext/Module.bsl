...
Функция ЗаписатьХранилищеЗначенияВАртефакт(Контейнер, Знач ЗначениеХранилища)
	...
	ПреобразованныеДанные = ПреобразоватьВДвоичныеДанные(ЗначениеХранилища);
	
	ИсходныйТип = Неопределено;
	Если ПреобразованныеДанные <> ЗначениеХранилища Тогда
		ИсходныйТип = СериализаторXDTO.XMLТипЗнч(ЗначениеХранилища);
	КонецЕсли;
	
	ВыгружатьКакБинарное = ТипЗнч(ПреобразованныеДанные) = Тип("ДвоичныеДанные");
	
	Если ВыгружатьКакБинарное Тогда
		
		Возврат ЗаписатьБинарноеХранилищеЗначенияВАртефакт(Контейнер, ПреобразованныеДанные, ИсходныйТип);
		
	Иначе
		
		Возврат ЗаписатьСериализуемоеХранилищеЗначенияВАртефакт(Контейнер, ПреобразованныеДанные);
...
//	ОбъектXDTO - артефакт.
//
Функция ЗаписатьБинарноеХранилищеЗначенияВАртефакт(Контейнер, Знач ЗначениеХранилища, ИсходныйТип = Неопределено)
	...
	Если ТипЗнч(ИсходныйТип) = Тип("ТипДанныхXML") Тогда
		ИсходныйТипXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(Пакет(), "OriginType"));
		ИсходныйТипXDTO.URI = ИсходныйТип.URIПространстваИмен;
		ИсходныйТипXDTO.Type = ИсходныйТип.ИмяТипа;
		
		ОписаниеЗначения.OriginType = ИсходныйТипXDTO;
	КонецЕсли;
...
Функция ПреобразоватьВДвоичныеДанные(ЗначениеХранилища)
	
	ПреобразованныеДанные = ЗначениеХранилища;
	
	Если ТипЗнч(ЗначениеХранилища) = Тип("ГрафическаяСхема") Тогда
		Поток = Новый ПотокВПамяти;
		ЗначениеХранилища.Записать(Поток);
		ПреобразованныеДанные = Поток.ЗакрытьИПолучитьДвоичныеДанные();
	КонецЕсли;
...
Процедура ЗагрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Если Артефакт.Data.Тип() = ТипБинарноеЗначение() Тогда
		...
		Если Артефакт.Data.Свойства().Получить("OriginType") <> Неопределено
				И Артефакт.Data.Установлено("OriginType") Тогда
				
			ТипЗначения = СериализаторXDTO.ИзXMLТипа(Артефакт.Data.OriginType.Type, Артефакт.Data.OriginType.URI);
			Значение = ПреобразоватьИзДвоичныхДанных(Значение, ТипЗначения);
		КонецЕсли;
...
Функция ПреобразоватьИзДвоичныхДанных(ДвоичныеДанные, ТипЗначения)
	
	ПреобразованныеДанные = ДвоичныеДанные;
	
	Если ТипЗначения = Тип("ГрафическаяСхема") Тогда
		Поток = ДвоичныеДанные.ОткрытьПотокДляЧтения();
		Схема = Новый ГрафическаяСхема;
		Схема.Прочитать(Поток);
		
		ПреобразованныеДанные = Схема;
	КонецЕсли;
