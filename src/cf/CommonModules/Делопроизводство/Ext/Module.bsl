...
// Формирует наименование дела
Функция НаименованиеДела(ДелоОбъект) Экспорт
...
Функция ПолучитьДанныеФайловДляСписка(Файлы, ВладелецФайла = Неопределено, ПоказыватьУдаленныеФайлы = Ложь, ОтметитьЗахваченныеФайлы = Ложь) Экспорт 
	...
	Если ТипЗнч(Файлы) = Тип("Массив") И Файлы.Количество() = 0 Тогда 
		...
		ДанныеФайлов.Колонки.Добавить("СтатусРаспознаванияТекста");
	...
	|		ПО (ВерсииФайлов.Ссылка = Файлы.ТекущаяВерсия)
	|ГДЕ
...
// Проверяет действие ставки НДС на дату в таблице товаров и услуг внутреннего документа.
//
// Параметры:
//  СтавкаНДС – ПеречислениеСсылка.СтавкиНДС
//  ДатаПроверки - Дата
//  СтрокаТаблицы - Число - номер строки таблицы товаров и услуг, в которой выполняется проверка.
//
// Возвращаемое значение
//  Строка - текст сообщения, который формируется в случае недействительной ставки НДС.
//
Функция СтавкаНДСДействительнаВТаблицеТоваровИУслуг(СтавкаНДС, ДатаПроверки, СтрокаТаблицы) Экспорт
	
	ТекстСообщения = "";
	
	Если (СтавкаНДС = Перечисления.СтавкиНДС.НДС5 Или СтавкаНДС = Перечисления.СтавкиНДС.НДС7)
		И ДатаПроверки < ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавокНДС5и7() Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Некорректный срок действия ставки НДС в строке %1. %2 действует с %3.'"),
			СтрокаТаблицы + 1,
			СтавкаНДС,
			Формат(ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавокНДС5и7(), "ДЛФ=D"));
	ИначеЕсли СтавкаНДС = Перечисления.СтавкиНДС.НДС18
		И (ДатаПроверки > ДелопроизводствоКлиентСервер.ДатаПрекращенияДействияСтавкиНДС18()
			Или ДатаПроверки < ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС18()) Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Некорректный срок действия ставки НДС в строке %1. %2 действует с %3 по %4.'"),
			СтрокаТаблицы + 1,
			СтавкаНДС,
			Формат(ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС18(), "ДЛФ=D"),
			Формат(ДелопроизводствоКлиентСервер.ДатаПрекращенияДействияСтавкиНДС18(), "ДЛФ=D"));
	ИначеЕсли СтавкаНДС = Перечисления.СтавкиНДС.НДС20
		И (ДатаПроверки > ДелопроизводствоКлиентСервер.ДатаПрекращенияДействияСтавкиНДС20()
			Или ДатаПроверки < ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС20()) Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Некорректный срок действия ставки НДС в строке %1. %2 действует с %3 по %4.'"),
			СтрокаТаблицы + 1,
			СтавкаНДС,
			Формат(ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС20(), "ДЛФ=D"),
			Формат(ДелопроизводствоКлиентСервер.ДатаПрекращенияДействияСтавкиНДС20(), "ДЛФ=D"));
	ИначеЕсли СтавкаНДС = Перечисления.СтавкиНДС.НДС22
		И ДатаПроверки < ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС22() Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Некорректный срок действия ставки НДС в строке %1. %2 действует с %3.'"),
			СтрокаТаблицы + 1,
			СтавкаНДС,
			Формат(ДелопроизводствоКлиентСервер.ДатаНачалаДействияСтавкиНДС22(), "ДЛФ=D"));
	КонецЕсли;
