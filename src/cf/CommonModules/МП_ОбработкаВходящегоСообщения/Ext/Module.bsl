...
Процедура ЗаписатьФайлВерсия1(ЗначенияСвойств)
	...
	ИначеЕсли Не ТолькоЗаголовок И ДанныеФайла <> Неопределено Тогда
		...
		Файл = РаботаСФайламиВнешнийВызов.СоздатьФайлНаОсновеФайлаНаДиске(ВладелецФайла,
			ИмяВременногоФайла,
			Имя,
			ИДФайла,
			Истина);
		...
		ИначеЕсли ВладелецТип = МП_СлужебныйПовтИсп.ТипПроцессИсполнения() Или
			...
			ПредметыПроцесса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВладелецФайла, "Предметы");
			СуществующиеИменаПредметов = ПредметыПроцесса.Выгрузить().ВыгрузитьКолонку("ИмяПредмета");
			
			ИмяПредмета =
				МультипредметностьВызовСервера.ПолучитьСсылкуНаИмяПредметаПоСсылкеНаПредмет(Файл,
				СуществующиеИменаПредметов);
...
Процедура ЗаписатьПеренаправлениеЗадачиВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ИсполнителиРолейИДелегаты.ИсполнительДелегат = &Исполнитель
		...
		|	Задачи.ВерсияДанных КАК ВерсияДанных
		|ИЗ
		|	ВТТекущиеРоли КАК ВТТекущиеРоли
		|		ЛЕВОЕ СОЕДИНЕНИЕ Задача.ЗадачаИсполнителя КАК Задачи
		|		ПО (ВТТекущиеРоли.РольПользователь = Задачи.Исполнитель)
		|ГДЕ
		...
		|	ВТТекущиеРоли КАК ВТТекущиеРоли
		|		ЛЕВОЕ СОЕДИНЕНИЕ Задача.ЗадачаИсполнителя КАК Задачи
		|		ПО (ВТТекущиеРоли.РольПользователь = Задачи.РольИсполнителя)
		|ГДЕ
		|	Задачи.Ссылка = &Ссылка";
