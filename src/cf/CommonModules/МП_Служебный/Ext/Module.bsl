...
Функция HTMLОписаниеМероприятия(Мероприятие) Экспорт
	...
	Если ПараметрыПечати.ВыводитьДатуИМестоПроведения Тогда 
		...
		Если ЗначениеЗаполнено(ВыборкаЗаголовок.МестоПроведения) Тогда
	...
	Если ПараметрыПечати.ВыводитьОрганизатора И ЗначениеЗаполнено(ВыборкаЗаголовок.Организатор) Тогда
		// Вывод организатора
	...
	Если ЗначениеЗаполнено(ВыборкаЗаголовок.Описание) Тогда
		// Вывод описания
		НачатьГруппировкуHTML(РезультатHTML, НСтр("ru = 'Описание:'; en = 'Description'"));

		РезультатHTML = РезультатHTML + Строка(ВыборкаЗаголовок.Описание);
