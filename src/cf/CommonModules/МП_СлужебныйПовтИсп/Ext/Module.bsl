...
Функция ВерсияБольше2_2_5(МобильноеПриложение) Экспорт
	...
	НомерВерсии = НомерВерсии(МобильноеПриложение);
	
	Возврат НомерВерсии > 5;
	
КонецФункции

//Возвращает номер версии приложения (числовое значение после второй точки)
// Параметры:
//  МобильноеПриложение - СправочникСсылка.ПользователиМобильногоПриложения - Мобильное приложение
// Возвращаемое значение:
//  Число - Номер версии
//
Функция НомерВерсии(МобильноеПриложение) Экспорт
	
	Версия = РегистрыСведений.СведенияОМобильныхКлиентах.ВерсияМобильногоПриложения(МобильноеПриложение);
	
	Если Не ЗначениеЗаполнено(Версия) Тогда
		Возврат 0;
	КонецЕсли;
	
	ВерсияМассив = СтрРазделить(Версия, ".");
	
	НомерПодредакции = Число(ВерсияМассив[1]);
	Если НомерПодредакции < 2 Тогда
		Возврат 0;
	КонецЕсли;
	
	НомерВерсии = Число(ВерсияМассив[2]);
