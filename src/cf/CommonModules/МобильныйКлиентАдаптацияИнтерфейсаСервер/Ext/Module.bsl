	...
	Если Параметры.ЭлементАктивныеЗадачи <> Неопределено Тогда
		...
		Если ЭлементАктивныеЗадачи <> Неопределено Тогда
			...
			Если СписокАктивныхЗадачНомерФлага <> Неопределено Тогда
				УстановитьПолюУсловноеОформлениеСкрытНеВидим(
...
#Область ОтображениеСтраницИНоваяСтраницаПредметы
	...
	Если Параметры.ЭлементГруппаСтраницы <> Неопределено Тогда
		...
		Если Параметры.ЭлементПредметы <> Неопределено
			...
			// Включим использование текущей строки.
			ЭлементПредметы.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
...
Процедура ВыровнятьЭлементФормы(Элемент) Экспорт
	...
	Если ТипЗнч(Элемент) <> Тип("ПолеФормы") И 
			ТипЗнч(Элемент) <> Тип("ДекорацияФормы") Тогда
		Возврат;
	КонецЕсли;
...
//
// Параметры:
//  Таблица - ДанныеФормыКоллекция - Представление таблицы на форме;
