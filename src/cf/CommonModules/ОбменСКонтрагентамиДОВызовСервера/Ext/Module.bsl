	...
	Для Каждого Документ Из ДокументыДО Цикл
		...
		Попытка
			...
			Если МассивЭД.Количество() > 0 Тогда
				...
				ЭлектронныеДокументыЭДОСлужебный.ВыполнитьДействияЭДО(ПараметрыВыполненияДействийЭДО);
...
Функция ДанныеДокументовДляОбработкиВходящих(Документы)
	...
	Для Каждого Документ Из Документы Цикл
		...
		ДанныеДокумента.Вставить("ФорматОсновногоТитула", Неопределено);
	...
	Пока ВыборкаДанныхЭДО.Следующий() Цикл
		...
		ДанныеДокумента.ФорматОсновногоТитула = ВыборкаДанныхЭДО.ФорматОсновногоТитула;
...
Функция ТребуетсяСозданиеОтветногоТитула(ДанныеДокумента)
	...
	Возврат ДанныеДокумента.ТребуетсяОтветнаяПодпись
			И НЕ НеТребуетсяРучноеСозданиеОтветногоТитула(ДанныеДокумента.ФорматОсновногоТитула)
			И РегламентыЭДО.ЕстьИнформацияПолучателя(ДанныеДокумента.ТипРегламентаЭДО)
			И Не ДанныеДокумента.ОтветныйТитулСоздан;
	
КонецФункции

Функция НеТребуетсяРучноеСозданиеОтветногоТитула(ФорматОсновногоТитула)
	
	ПоддерживаемыеФорматы = ЭлектронныеДокументыЭДО.ПоддерживаемыеФорматы();
	
	НеТребуетсяРучноеСозданиеОтветногоТитула =
		ФорматОсновногоТитула = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПродавца
		ИЛИ ФорматОсновногоТитула = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПродавца;
	
	Возврат НеТребуетсяРучноеСозданиеОтветногоТитула;
