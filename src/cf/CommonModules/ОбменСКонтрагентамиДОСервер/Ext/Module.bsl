...
Функция СоздатьДокументПоСообщениюЭДО(СообщениеЭДО, ВходящийЭД = Неопределено, ФайлЭД = Неопределено) Экспорт
	...
	Попытка
		...
		УдалитьОшибкиОтраженияВходящегоЭДО(ВходящийЭД);
...
// Удаляет ошибки отражения входящего ЭДО и возвращает статус документа к первоначальному
// 
// Параметры:
//  ВходящийЭДО - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
Процедура УдалитьОшибкиОтраженияВходящегоЭДО(ВходящийЭДО)
	
	РегистрыСведений.ОшибкиЭДОКИсправлению.УдалитьЗаписьОбОшибке(ВходящийЭДО);
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЭлектронныйДокументВходящийЭДО.Ссылка КАК ВходящийЭДО,
		|	СообщениеЭДО.Ссылка КАК СообщениеЭДО,
		|	ЭлектронныйДокументВходящийЭДО.Остановлен КАК ДокументОстановлен,
		|	СообщениеЭДО.Статус КАК СтатусСообщения,
		|	СостоянияДокументовЭДО.Состояние КАК СостояниеДокументаЭДО
		|ИЗ
	...
	Запрос.УстановитьПараметр("ДокументЭДО", ВходящийЭДО);
	
	ОбработанныеДокументы = Новый Соответствие();
	ВозвращенныеСостоянияЭДО = Новый Соответствие();
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		ДокументЭДО = Выборка.ВходящийЭДО;
		
		Если Не ЗначениеЗаполнено(ДокументЭДО) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не Выборка.ДокументОстановлен
			И Выборка.СтатусСообщения <> Перечисления.СтатусыСообщенийЭДО.НеПолучен
			И Выборка.СостояниеДокументаЭДО <>
				Перечисления.СостоянияДокументовЭДО.ТребуетсяИсправлениеОшибкиПередачи Тогда
		...
		Попытка
			...
			Если Выборка.ДокументОстановлен И ОбработанныеДокументы[ДокументЭДО] <> Истина Тогда
				...
				ОбработанныеДокументы[ДокументЭДО] = Истина;
				
			КонецЕсли;
			
			Если Выборка.СтатусСообщения = Перечисления.СтатусыСообщенийЭДО.НеПолучен Тогда
				
				СообщениеОбъект = Выборка.СообщениеЭДО.ПолучитьОбъект();
			...
			Если Выборка.СостояниеДокументаЭДО =
				...
				ВозвращенныеСостоянияЭДО[ДокументЭДО] = Истина;
		...
		Исключение
			...
			ТекстСообщения =
				СтрШаблон(НСтр("ru = 'Не удалось вернуть статус к отражению документа %1 по причине:'"),
					ДокументЭДО)
...
Функция ПолучитьДанныеИзФайлаЭД(ДанныеВыполненияОперации, Ошибки = Неопределено)
	...
	РеквизитыЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ВходящийЭД,
		"ВидДокумента, Организация, Контрагент, ИдентификаторКонтрагента");
	...
	ДанныеДляЗаполнения.Вставить("ИдентификаторОтправителя", РеквизитыЭД.ИдентификаторКонтрагента);
...
Функция ОпределитьПараметрыПоПравиламУчета(ДанныеВыполненияОперации, Ошибки = Неопределено)
	...
	Если Делопроизводство.ИспользоватьВидыВнутреннихДокументов() 
		...
		ДанныеПриглашения = ДанныеПриглашенияПоДокументуЭДО(ДанныеВыполненияОперации.ДанныеЭД.ВходящийЭД);
		
		Если ЗначениеЗаполнено(ДанныеПриглашения.Организация) И ЗначениеЗаполнено(ДанныеПриглашения.Контрагент) Тогда
			
			ПараметрыДокументаПоВидуЭДО = 
				РегистрыСведений.ПравилаУчетаВидовЭДДО.ПараметрыДокументаПоВидуЭДО(
					ДанныеДляЗаполнения.ВидДокументаЭДО,
					ДанныеПриглашения.Организация,
					ДанныеПриглашения.Контрагент);
...
// Возвращает организацию и контрагента приглашения к обмену по документу ДО
// 
// Параметры:
//  ДокументЭДО - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
// 
// Возвращаемое значение:
//  Структура - Данные приглашения по документу ЭДО:
// * Организация - СправочникСсылка.Организации - 
// * Контрагент - СправочникСсылка.Контрагенты - 
Функция ДанныеПриглашенияПоДокументуЭДО(ДокументЭДО)
	
	ОрганизацияИКонтрагент = Новый Структура;
	ОрганизацияИКонтрагент.Вставить("Организация", Справочники.Организации.ПустаяСсылка());
	ОрганизацияИКонтрагент.Вставить("Контрагент", Справочники.Контрагенты.ПустаяСсылка());
	...
	Запрос.Текст =
		...
		|		ПО ЭлектронныйДокументВходящийЭДО.ИдентификаторОрганизации = ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторОрганизации
		|			И ЭлектронныйДокументВходящийЭДО.ИдентификаторКонтрагента = ПриглашенияКОбменуЭлектроннымиДокументами.ИдентификаторКонтрагента
		|ГДЕ
		|	ЭлектронныйДокументВходящийЭДО.Ссылка = &ДокументЭДО";
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.Организация) И ЗначениеЗаполнено(Выборка.Контрагент) Тогда
			ЗаполнитьЗначенияСвойств(ОрганизацияИКонтрагент, Выборка);
			Возврат ОрганизацияИКонтрагент;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьЭДПоГотовымДокументамДО(Знач ПараметрыФормирования = Неопределено) Экспорт
	...
	ОбменСКонтрагентамиДОСобытия.ПередСозданиемОбъектовЭДОПоДокументамДО(
...
Функция СформироватьФормализованныйЭД(КонтекстФормирования)
	...
	РезультатФормирования = ЭлектронныеДокументыЭДОСлужебный.ВыполнитьДействияЭДО(ПараметрыВыполненияДействийПоЭДО);
...
Функция СкопироватьПодписиВФайлЭД(КонтекстФормирования)
	...
	ДвоичныеДанныеФайловЭД = РаботаСФайлами.ДвоичныеДанныеФайлов(ФайлыЭД);
	...
	Для Каждого Подпись Из УстановленныеПодписи Цикл
		...
		Сертификат = ЭлектроннаяПодпись.СсылкаНаСертификат(Подпись.Сертификат.Получить());
		
		ОписаниеПодписанногоДокумента = ЭлектронныеДокументыЭДОСлужебный.НовоеОписаниеПодписанногоСообщения();
		...
		СертификатОписания = ОписаниеПодписанногоДокумента.СвойстваПодписи.Сертификат;
		Если ТипЗнч(СертификатОписания) = Тип("ХранилищеЗначения") Тогда
			ОписаниеПодписанногоДокумента.СвойстваПодписи.Сертификат = СертификатОписания.Получить();
		КонецЕсли;
	...
	РезультатВыполнения = ЭлектронныеДокументыЭДО.НовыйРезультатДействийПоЭДО(КонтекстДиагностики);
	РезультатВыполнения.Итог = ИтогДействийПоЭДО;
	ВыбранныеСертификаты = ОбщегоНазначения.ВыгрузитьКолонку(ПодписанныеДокументы, "ВыбранныйСертификат", Истина);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПараметрыВыполненияДействийЭДО.ВыбранныеСертификаты, ВыбранныеСертификаты, Истина);
	ЭлектронныеДокументыЭДОСлужебный.ОбработатьПодписанныеСообщения(ПодписанныеДокументы, РезультатВыполнения);
	ЭлектронныеДокументыЭДОСлужебный.ПродолжитьВыполнениеДействийПоЭДО(ПараметрыВыполненияДействийЭДО, РезультатВыполнения);
...
Функция ТаблицыДляПроверкиВозможностиСозданияИсходящегоЭДО(Документ)
	...
	ТекстОсновногоЗапроса =
		...
		|	ВЫРАЗИТЬ(ДокументыДОСтороны.Сторона КАК Справочник.Организации).ИНН КАК ИННОрганизации
		...
		|	0 КАК Приоритет
		|ПОМЕСТИТЬ УчетныеЗаписиЭДООрганизацииПоИНН
		|ИЗ
		...
		|	ДокументСОрганизацией КАК ДокументСОрганизацией
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетныеЗаписиЭДО КАК УчетныеЗаписиЭДО
		|		ПО ДокументСОрганизацией.ИННОрганизации = УчетныеЗаписиЭДО.Организация.ИНН;
		|/////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ Минимум(Приоритет) КАК Приоритет ПОМЕСТИТЬ МинимальныйПриоритетУчетныхЗаписей ИЗ УчетныеЗаписиЭДООрганизацииПоИНН;
		|/////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		|	УчетныеЗаписиЭДООрганизацииПоИНН.Документ КАК Документ,
		|	УчетныеЗаписиЭДООрганизацииПоИНН.Организация КАК Организация,
		|	УчетныеЗаписиЭДООрганизацииПоИНН.ИдентификаторЭДО КАК ИдентификаторЭДО,
		|	УчетныеЗаписиЭДООрганизацииПоИНН.Приоритет КАК Приоритет
		|ПОМЕСТИТЬ УчетныеЗаписиЭДООрганизации
		|ИЗ
		|	УчетныеЗаписиЭДООрганизацииПоИНН ВНУТРЕННЕЕ СОЕДИНЕНИЕ МинимальныйПриоритетУчетныхЗаписей ПО
		|	УчетныеЗаписиЭДООрганизацииПоИНН.Приоритет = МинимальныйПриоритетУчетныхЗаписей.Приоритет
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
