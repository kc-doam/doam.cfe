...
Процедура ЗапланироватьЗадания()
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Попытка
			...
			Шаблон = МодульОчередьЗаданий.ШаблонПоИмени(Метаданные.РегламентныеЗадания.ОтложенноеОбновлениеИБ.Имя);
			Если ЗначениеЗаполнено(Шаблон) Тогда
				Задание = Справочники.ОчередьЗаданий.СоздатьЭлемент();
				Задание.Шаблон = Шаблон;
				Задание.ЗапланированныйМоментЗапуска = ТекущаяУниверсальнаяДата();
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				Нстр("ru = 'Не удалось добавить задания в очередь. Причина:
					|%1'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
