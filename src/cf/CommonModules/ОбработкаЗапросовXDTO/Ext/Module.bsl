...
Функция ПолучитьОбъекты(Сообщение) Экспорт
	...
	Попытка
		...
		Для Каждого ОбъектИд Из Сообщение.objectIds Цикл
			...
			ОбъектXDTO = ОбработкаЗапросовXDTOПереопределяемый.ПолучитьОбъектПоObjectID(ОбъектИд, Сообщение.columnSet);
			
			Если ОбъектXDTO = Неопределено Тогда
				
				Если ОбъектИд.type = "DMFile" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOФайлы.ПолучитьКарточкуФайла(ОбъектИд.id, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMFileVersion" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOФайлы.ПолучитьВерсиюФайла(ОбъектИд.id, Сообщение.columnSet);
				...
				ИначеЕсли ОбъектИд.type = "DMBusinessProcessTask"
					...
					Если ВыполняетсяЗапросHTMLПредставления Тогда
						ОбъектXDTO = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ПолучитьHTMLФормы(ОбъектИд);
					Иначе
						ОбъектXDTO = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ПолучитьЗадачуБизнесПроцесса(
							ОбъектИд,
				...
				ИначеЕсли ОбъектИд.type = "DMBusinessProcessPerformance"
					...
					ОбъектXDTO = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ПолучитьБизнесПроцесс(ОбъектИд);
					
				ИначеЕсли ОбъектИд.type = "DMBusinessProcessExecutorRole" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ПолучитьРольИсполнителей(ОбъектИд);
					
				ИначеЕсли ОбъектИд.type = "DMIncomingDocument" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьВходящийДокумент(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMOutgoingDocument" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьИсходящийДокумент(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMInternalDocument" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьВнутреннийДокумент(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMIncomingDocumentTemplate" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьШаблонВходящегоДокумента(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMOutgoingDocumentTemplate" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьШаблонИсходящегоДокумента(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMInternalDocumentTemplate" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьШаблонВнутреннегоДокумента(ОбъектИд, Сообщение.columnSet);
				...
				ИначеЕсли ОбъектИд.type = "DMInternalDocumentType"
					...
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьВидДокумента(ОбъектИд);
					
				ИначеЕсли ОбъектИд.type = "DMInternalDocumentFolder" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьПапкуВнутреннихДокументов(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMSubdivision" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьПодразделение(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMMeasurementUnit" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьЕдиницуИзмерения(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMProduct" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьНоменклатуру(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMCurrency" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьВалюту(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMBank" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьБанк(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMBankAccount" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьБанковскийСчет(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMContactInformationKind" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьВидКонтактнойИнформации(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMUser" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьПользователя(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMContactPerson" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьКонтактноеЛицо(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMPrivatePerson" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьФизическоеЛицо(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMOrganization" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьОрганизацию(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMCorrespondent" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьКонтрагента(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMDailyReport" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOУчетВремени.ПолучитьЕжедневныйОтчет(ОбъектИд);
					
				ИначеЕсли ОбъектИд.type = "DMWeeklyReport" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOУчетВремени.ПолучитьЕженедельныйОтчет(ОбъектИд);
					
				ИначеЕсли ОбъектИд.type = "DMApplicationNotification" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTO.ПолучитьУведомлениеПрограммы(ОбъектИд, Сообщение.columnSet);
				...
				ИначеЕсли ОбъектИд.type = "DMCaseFilesListSections" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьРазделНоменклатурыДел(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMCaseFilesCatalog" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьНоменклатуруДел(ОбъектИд, Сообщение.columnSet);
					
				ИначеЕсли ОбъектИд.type = "DMCaseFilesDossiers" Тогда
					ОбъектXDTO = ОбработкаЗапросовXDTOДокументы.ПолучитьДелоХраненияДокументов(ОбъектИд, Сообщение.columnSet);
					
				Иначе
					ОбъектXDTO = ПолучитьНеОписанныйОбъект(ОбъектИД);
			...
			Если СвойствоУстановлено(ОбъектXDTO, "ObjectID")
					И Не ПустаяСтрока(ОбъектXDTO.ObjectID.ID)
					И ОбъектXDTO.ObjectID.type <> "DMFile"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessPerformance"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessPerformance_1.2.1.11"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessAcquaintance"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessOrder"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessConsideration"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessRegistration"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessApproval"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessApproval_1.2.1.11"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessConfirmation"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessInternalDocumentProcessing"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessIncomingDocumentProcessing"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessOutgoingDocumentProcessing"
					И ОбъектXDTO.ObjectID.type <> "DMBusinessProcessIssuesSolution" Тогда
				ПротоколированиеРаботыПользователей.ЗаписатьОткрытие(ПолучитьСсылкуПоObjectID(ОбъектXDTO.ObjectID));
			КонецЕсли;
...
Процедура ЗаполнитьСвойствоXDTOИзРеквизита(ОбъектXDTO, ИмяСвойстваXDTO, Реквизит) Экспорт
	...
	ЗначенияКлючевыхРеквизитов = Неопределено;
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипРеквизита) Тогда
		...
		// Заполнение ключевых реквизитов.
		Если ((ТипРеквизита = Тип("СправочникСсылка.Организации")
						И ТипСвойства = ФабрикаXDTO.Тип("http://www.1c.ru/dm", "DMOrganization"))
					Или (ТипРеквизита = Тип("СправочникСсылка.Контрагенты")
						И ТипСвойства = ФабрикаXDTO.Тип("http://www.1c.ru/dm", "DMCorrespondent")))
				И ЧтениеРазрешено(Реквизит, "ИНН, КПП, ЮрФизЛицо", ЗначенияКлючевыхРеквизитов) Тогда
			
			ЗаполнитьСвойствоXDTOИзРеквизита(ЗначениеСвойства, "inn", ЗначенияКлючевыхРеквизитов.ИНН);
			ЗаполнитьСвойствоXDTOИзРеквизита(ЗначениеСвойства, "kpp", ЗначенияКлючевыхРеквизитов.КПП);
			ЗаполнитьСвойствоXDTOИзРеквизита(ЗначениеСвойства, "legalPrivatePerson", ЗначенияКлючевыхРеквизитов.ЮрФизЛицо);
			
		ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.Пользователи")
				И ТипСвойства = ФабрикаXDTO.Тип("http://www.1c.ru/dm", "DMUser")
				И ЧтениеРазрешено(Реквизит, "ФизЛицо, Подразделение", ЗначенияКлючевыхРеквизитов) Тогда
			
			ЗаполнитьСвойствоXDTOИзРеквизита(ЗначениеСвойства, "privatePerson", ЗначенияКлючевыхРеквизитов.ФизЛицо);
			ЗаполнитьСвойствоXDTOИзРеквизита(ЗначениеСвойства, "subdivision", ЗначенияКлючевыхРеквизитов.Подразделение);
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMUpdateResponse или DMError.
//
Функция ЗаписатьОбъекты(Сообщение)
	...
	Для Каждого Объект Из Сообщение.objects Цикл
		...
		Если СвойствоУстановлено(Объект, "ObjectID") И Не ПустаяСтрока(Объект.ObjectID.ID) Тогда
			ПротоколированиеРаботыПользователей.ЗаписатьИзменение(
				ПолучитьСсылкуПоObjectID(Объект.ObjectID),
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMCreateRequest или DMError.
//
Функция Создать(Сообщение)
	...
	НовыйОбъектXDTO = ОбработкаЗапросовXDTOПереопределяемый.Создать(Сообщение);
	
	Если НовыйОбъектXDTO = Неопределено Тогда
		
		Если ПроверитьТип(Сообщение.object, "DMIncomingDocument") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьВходящийДокумент(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMOutgoingDocument") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьИсходящийДокумент(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMInternalDocument") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьВнутреннийДокумент(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMSubdivision") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьПодразделение(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMMeasurementUnit") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьЕдиницуИзмерения(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMProduct") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьНоменклатуру(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMCurrency") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьВалюту(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMBank") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьБанк(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMBankAccount") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьБанковскийСчет(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMUser") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьПользователя(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMContactPerson") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьКонтактноеЛицо(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMPrivatePerson") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьФизическоеЛицо(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMOrganization") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьОрганизацию(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMCorrespondent") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьКонтрагента(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMCaseFilesListSections") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьРазделНоменклатурыДел(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMCaseFilesCatalog") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьНоменклатуруДел(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMCaseFilesDossiers") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьДелоХраненияДокументов(Сообщение);
		...
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMBusinessProcessPerformance")
			...
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOБизнесПроцессыИЗадачи.ЗаписатьБизнесПроцесс(Сообщение.object, Истина);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMDailyReport") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOУчетВремени.СоздатьЕжедневныйОтчет(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMWeeklyReport") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOУчетВремени.СоздатьЕженедельныйОтчет(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMActualWork") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOУчетВремени.СоздатьЗаписьФактическихТрудозатрат(Сообщение);
			
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMInternalDocumentFolder") Тогда
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьПапкуВнутреннихДокументов(Сообщение);
		...
		ИначеЕсли ПроверитьТип(Сообщение.object, "DMInternalDocumentType") 
			...
			НовыйОбъектXDTO = ОбработкаЗапросовXDTOДокументы.СоздатьВидДокумента(Сообщение);
	...
	Если ПроверитьТип(НовыйОбъектXDTO, "DMError") Или ПроверитьТип(НовыйОбъектXDTO, "DMOK") Тогда
		Возврат НовыйОбъектXDTO
	КонецЕсли;
	
	Ответ.object = НовыйОбъектXDTO;
	
	Если СвойствоУстановлено(НовыйОбъектXDTO, "ObjectID") И Не ПустаяСтрока(НовыйОбъектXDTO.ObjectID.ID) Тогда
		ПротоколированиеРаботыПользователей.ЗаписатьСоздание(ПолучитьСсылкуПоObjectID(НовыйОбъектXDTO.ObjectID), Истина);
	КонецЕсли;
...
//   ЗначенияРеквизитов - Структура, ВыборкаИзРезультатаЗапроса - неявно возвращаемое значение, содержит
//     значения реквизитов.
//
// Возвращаемое значение:
//   Булево
//
Функция ЧтениеРазрешено(Ссылка, Реквизиты = "", ЗначенияРеквизитов = Неопределено)
	...
	МассивТекстЗапросаПолей = Новый Массив;
	Для Каждого Реквизит Из МассивРеквизитов Цикл
		МассивТекстЗапросаПолей.Добавить(СтрШаблон("ПроверяемыйОбъект.%1 КАК %1", СокрЛП(Реквизит)));
	КонецЦикла;
	...
	Запрос = Новый Запрос(
		...
		|	&ТекстЗапросаПолей
		|ИЗ
		|	&ПолноеИмя КАК ПроверяемыйОбъект
		|ГДЕ
		|	ПроверяемыйОбъект.Ссылка = &Ссылка");
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ТекстЗапросаПолей",
		СтрСоединить(МассивТекстЗапросаПолей, ", "));
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ПолноеИмя",
	...
	Возврат ЗначенияРеквизитов.Следующий();
