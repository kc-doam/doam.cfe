...
//
// Параметры:
//   ЗадачаXDTO - ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcessTask
...
//   Ответ - ОбъектXDTO - объект XDTO типа DMGetObjectListResponse, неявно возвращаемое значение.
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetTasksTreeRequest
//   Ответ - ОбъектXDTO - объект XDTO типа DMGetTasksTreeResponse, неявно возвращаемое значение
//   ИмяТипа - Строка - полное имя типа вида БизнесПроцесс.<...>.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetTasksTreeResponse или DMError
//
...
//
// Параметры:
//   ОбъектИд - ОбъектXDTO - объект XDTO типа DMObjectID.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMObjectHTMLPresentation, содержащий HTML-представление задачи
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcess, с заполненными свойствами
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessExecutorRole, заполненные данными роли
//
...
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа DMBusinessProcess<...> - источник данных заполнения.
//
		...
		Для Каждого СтрокаПредмета Из Предметы Цикл
			...
			Если Не МультипредметностьКлиентСервер.ЭтоКомплексныйПроцесс(БизнесПроцесс) Тогда
				...
				Для Каждого СтрокаДействия Из ДействияПроцесса Цикл
					СтруктураПоиска = Новый Структура("ИмяПредмета, ТочкаМаршрута", СтрокаПредмета.ИмяПредмета, СтрокаДействия);
					СтрокиПредметовЗадач = БизнесПроцесс.ПредметыЗадач.НайтиСтроки(СтруктураПоиска);
					...
					Иначе
						СтрокаПредметаЗадачи = СтрокиПредметовЗадач[0];
					КонецЕсли;
			...
			Иначе // комплексный процесс
				...
				ДействияПроцесса = Мультипредметность.ПолучитьДействияКомплексногоПроцесса(БизнесПроцесс.Ссылка);
				Для Каждого СтрокаДействия Из ДействияПроцесса Цикл
					СтруктураПоиска = Новый Структура("ИмяПредмета", СтрокаПредмета.ИмяПредмета);
					СтруктураПоиска.Вставить("ТочкаМаршрута", СтрокаДействия.ТочкаМаршрута);
					СтруктураПоиска.Вставить("ИдентификаторЭтапа",
						?(СтрокаДействия.ИдентификаторЭтапа = Неопределено,
							Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"),
							СтрокаДействия.ИдентификаторЭтапа));
					СтрокиПредметовЗадач = БизнесПроцесс.ПредметыЗадач.НайтиСтроки(СтруктураПоиска);
					Если СтрокиПредметовЗадач.Количество() = 0 Тогда
						СтрокаПредметаЗадачи = БизнесПроцесс.ПредметыЗадач.Добавить();
					Иначе
...
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа DMBusinessProcessTaskExecutor - источник данных для заполнения
//
...
//   ИсполнительXDTO - ОбъектXDTO - объект XDTO типа DMBusinessProcessTaskExecutor - источник данных для заполнения
//
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcess.
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMLaunchBusinessProcessResponse или DMError
//
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetNewBusinessProcessRequest
//   ТипОтвета - Строка - имя XDTO-типа желаемого ответа, обычно DMGetNewBusinessProcessResponse
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа ТипОтвета, обычно DMGetNewBusinessProcessResponse
//
...
// 
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetBusinessProcessTemplatesRequest
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetBusinessProcessTemplatesResponse
// 
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetBusinessProcessTemplatesTreeRequest
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetBusinessProcessTemplatesTreeResponse 
//
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetTemplateHintRequest
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetTemplateHintResponse или DMError
//
...
//
// Параметры:
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcessTask
// Возвращаемое значение:
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMOK в случае успеха или DMError в противном случае
//
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetBusinessProcessByTemplateRequest
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetBusinessProcessByTemplateResponse
//
...
//
// Параметры:
//   ОбъектИд - ОбъектXDTO - объект XDTO типа ObjectID, описывающий задачу
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcessTask
//
...
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа, наследующего DMBusinessProcess.
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMGetApprovalSheetRequest
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMGetApprovalSheetResponse или DMError
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMApprovalStateRecord.
//
...
//   ИсполнительXDTO - ОбъектXDTO - объект XDTO типа DMUser - исполнитель к заполнению в задаче
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessPerformance
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessPerformance_1.2.1.11
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessAcquaintance
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessOrder
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessConsideration
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessRegistration
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessApproval
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessApproval_1
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessConfirmation
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessIncomingDocumentProcessing
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessInternalDocumentProcessing
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessOutgoingDocumentProcessing
//
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcessIssuesSolution
//
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessPerformance
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
Функция ЗаписатьБПИсполнение(Объект, СоздатьНовый)
	...
	// Исполнители.
	Если Не БПОбъект.Стартован И Не БПОбъект.Завершен Тогда
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessPerformance_1.2.1.11
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
//   БизнесПроцессСсылка.Исполнение - ссылка на записанный процесс
	...
	Если Не БПОбъект.Стартован И Не БПОбъект.Завершен Тогда
		...
		Для Каждого Исполнитель Из Объект.performers Цикл
			...
			НоваяСтрока.СрокИсполнения =
				?(Исполнитель.personalDueDate = Неопределено,
					Дата(1, 1, 1),
					Исполнитель.personalDueDate);
			НоваяСтрока.Описание = Исполнитель.personalDescription;
			НоваяСтрока.НаименованиеЗадачи = Исполнитель.personalTaskName;
			НоваяСтрока.Ответственный = Исполнитель.responsible;
			НоваяСтрока.ПорядокИсполнения = ОбработкаЗапросовXDTO.ПолучитьСсылкуПоObjectID(
				Исполнитель.performanceOrder.ObjectID);
		КонецЦикла;
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessAcquaintance
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessOrder
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessConsideration
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessRegistration
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessApproval
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
Функция ЗаписатьБПСогласование(Объект, СоздатьНовый)
	...
	// Исполнители.
	Если Не БПОбъект.Завершен И (Не БПОбъект.Стартован Или ЭтоОтправкаНаПовторноеСогласование(БПОбъект)) Тогда
		...
		Для Каждого Исполнитель Из Объект.performers Цикл
			...
			Если Не Исполнитель.Установлено("dueDateSpecificationOption")
				...
				ПараметрыДлительности = СрокиИсполненияПроцессов.ПараметрыДлительностиИсполненияДляОпределенияТочногоСрока();
				...
				НоваяСтрока.СрокИсполнения = СрокиИсполненияПроцессов.СрокИсполненияПоДлительности(ПараметрыДлительности);
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessApproval_1.2.1.11
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
//   БизнесПроцессСсылка.Согласование - ссылка на записанный процесс
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessConfirmation
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessInternalDocumentProcessing
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessIncomingDocumentProcessing
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessOutgoingDocumentProcessing
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//
// Параметры:
//   Объект - ОбъектXDTO - объект XDTO типа DMBusinessProcessIssuesSolution
//   СоздатьНовый - Булево - Истина, если нужно создать новый объект
//
// Возвращаемое значение:
...
//   ОдинОбъектXDTO - ОбъектXDTO - объект XDTO типа DMGetTasksTreeResponse - заполняемый объект
//
...
//
// Параметры:
//   ObjectIDБизнесПроцесса - ОбъектXDTO - объект XDTO типа ObjectID, описывающий бизнес-процесс
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcess, частично заполненный и соответствующий запущенному процессу
//
...
//
// Параметры:
//   Query - ОбъектXDTO - объект XDTO типа DMGetTasksTreeQuery, описывающий условия отбора.
//
// Возвращаемое значение:
//   Массив из Структура:
...
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMBusinessProcess<...>
//
...
Процедура ПередатьЗначенияПредметовБизнесПроцессаЗадачи(Владелец, Объект,
	...
	Иначе
		...
		fillableTargetCollection = СоздатьОбъектXDTO("DMBusinessProcessTaskFillableTargetCollection");
	КонецЕсли;
	...
	Для Каждого СтрокаПредмета Из Предметы Цикл
		...
		Иначе
			Если Не ЗначениеЗаполнено(СтрокаПредмета.Предмет)
					И СтрокаПредмета.РольПредмета = Перечисления.РолиПредметов.Заполняемый Тогда
				target = СоздатьОбъектXDTO("DMBusinessProcessTaskFillableTarget");
			Иначе
		...
		target.role.ObjectID = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(СтрокаПредмета.РольПредмета);
		...
		Если Не ЗначениеЗаполнено(СтрокаПредмета.Предмет) Тогда
			Если Не ЭтоБизнесПроцесс И СтрокаПредмета.РольПредмета = Перечисления.РолиПредметов.Заполняемый Тогда
				target.requiredField = СтрокаПредмета.ОбязательноеЗаполнение;
				fillableTargetCollection.items.Добавить(target);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		...
		Если Не Объект.Установлено("target") Тогда // Совместимость со старыми клиентами.
			...
			Объект.target.ObjectID = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(СтрокаПредмета.Предмет);
		КонецЕсли;
		...
		Если СтрокаПредмета.ТипПредмета = Тип("СправочникСсылка.Файлы") Тогда
			...
			target.target.ObjectID = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(СтрокаПредмета.Предмет);
			...
			Если ЗначениеЗаполнено(Редактирует) Тогда
				...
				editingUser.ObjectID = ОбработкаЗапросовXDTO.ПолучитьObjectIDПоСсылке(Редактирует);
		...
		Иначе
			...
			// Для прочих объектов достаточно абстрактного DMObject.
	...
	Если Не ЭтоБизнесПроцесс Тогда
		Объект.fillableTargets = fillableTargetCollection;
	КонецЕсли;
...
Функция ЭтоОтправкаНаПовторноеСогласование(БПОбъект)
	
	Если БПОбъект.РезультатСогласования <> Перечисления.РезультатыСогласования.НеСогласовано Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТЗРезультат.Количество() <> 1 Тогда
		Возврат Ложь;
	ИначеЕсли ТЗРезультат[0].ТочкаМаршрута <> БизнесПроцессы.Согласование.ТочкиМаршрута.Ознакомиться Тогда
		Возврат Ложь;
	КонецЕсли;
