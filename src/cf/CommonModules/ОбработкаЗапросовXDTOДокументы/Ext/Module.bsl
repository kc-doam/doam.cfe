...
Функция ЗаписатьДанныеВнутреннегоДокумента(Документ, ОбъектXDTO, Ошибка)
	...
	ЗаполнятьПоШаблону = ЗаполнитьДокументПоШаблону(Документ, ОбъектXDTO);
	...
	Попытка
		...
		// срок действия
		Если (Документ.ДополнительныеСвойства.Свойство("ЭтоНовый")
					И Документ.ДополнительныеСвойства.ЭтоНовый
					И СрокДействияЗаполнен(Документ))
...
Функция СоздатьПользователя(Сообщение) Экспорт
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, "3.1.10.78") >= 0 Тогда
			МодульПользователиСлужебный.ОбновитьСоставГруппыВсеПользователи(
				Пользователь.Ссылка,
				МодульПользователиСлужебный.НовыеИзмененияСоставовГрупп());
		Иначе
			МодульПользователиСлужебный.ОбновитьСоставыГруппПользователей(Справочники.РабочиеГруппы.ВсеПользователи);
		КонецЕсли;
...
//   ОбъектXDTO - ОбъектXDTO - объект XDTO типа DMUser. Источник данных заполнения.
//
...
Функция НайтиСоздатьПользователя(ОбъектXDTO) Экспорт
	...
	Если СсылкаДО <> Неопределено Тогда
		...
		Если Не ОбъектДОЗаблокирован(ОбъектДО.Ссылка) Тогда
			...
			Если ВыполнитьЗаполнение Тогда
				ЗаблокированныеОбъектыДобавить(ОбъектДО.Ссылка);
				ЗаполнитьДанныеПользователя(ОбъектДО, ОбъектXDTO);
				Если ОбъектДО.Модифицированность() Тогда
					ОбъектДО.Записать();
				КонецЕсли;
				ОбъектДО.Разблокировать();
				ЗаблокированныеОбъектыУдалить(ОбъектДО.Ссылка);
			КонецЕсли;
		КонецЕсли;
	...
	// Стандартный поиск.
	Если Не ЗначениеЗаполнено(СсылкаДО) И ЗначениеЗаполнено(Наименование) Тогда
		...
		Если ОбработкаЗапросовXDTO.СвойствоУстановлено(ОбъектXDTO, "privatePerson") Тогда
			ФизЛицо = Справочники.ФизическиеЛица.НайтиПоНаименованию(ОбъектXDTO.privatePerson.name, Истина);
		КонецЕсли;
...
Процедура СвязатьПользователяСПодразделением(ОбъектДО, ОбъектXDTO)
	...
	Если ЗначениеЗаполнено(Подразделение) Тогда
		...
		Если МенеджерЗаписи.Подразделение <> Подразделение Тогда
			МенеджерЗаписи.Пользователь = ОбъектДО.Ссылка;
...
//
// Параметры:
//   Сообщение - ОбъектXDTO - объект XDTO типа DMAddDocumentRelationRequest.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMAddDocumentRelationResponse или DMError.
//
Функция СоздатьСвязьДокументов(Сообщение) Экспорт
	...
	Попытка
		...
		Иначе
			...
			// Тип связи не указан, выберем однозначно настроенную или, в отсутствие таковой, предопределенную.
		...
		// Обновим реквизиты связываемых документов.
		РеквизитыВладельцаИзменены = СвязиДокументов.УстановитьРеквизитыПриДобавленииСвязи(Документ,, ТипСвязи);
		...
		Если НастройкаСвязи <> Неопределено И ЗначениеЗаполнено(НастройкаСвязи.ТипОбратнойСвязи) Тогда
			СвязиДокументов.УстановитьРеквизитыПриДобавленииСвязи(СвязанныйДокумент,, НастройкаСвязи.ТипОбратнойСвязи);
		КонецЕсли;
...
Функция ЗарегистрироватьДокумент(Сообщение) Экспорт
	...
	Попытка
		ДокументСсылка = ОбработкаЗапросовXDTO.ПолучитьСсылкуПоObjectID(Сообщение.document.objectId);
		Документ = ДокументСсылка.ПолучитьОбъект();
		
		Попытка
			// В отсутствие подходящего нумератора будет выброшено исключение.
			Нумератор = Нумерация.ПолучитьНумераторДокумента(Документ);
		Исключение
			Нумератор = Неопределено;
		КонецПопытки;
		...
		Если Не ЗначениеЗаполнено(Нумератор) Тогда
			ВызватьИсключение НСтр("ru = 'Документ регистрируется вручную. Укажите номер и запишите документ.'");
		КонецЕсли;
		...
		Если Не Делопроизводство.ЗарегистрироватьПредмет(ДокументСсылка, СообщениеОбОшибке) Тогда
			ВызватьИсключение СообщениеОбОшибке;
		КонецЕсли;
...
Процедура ЗаполнитьСписокПодписейФайла(ТекущийОбъект, МассивПодписей) Экспорт
	...
	Если ТекущийОбъект.ПодписанЭП Тогда
		...
		РаботаСЭП.ПередЧтениемЭПОбъекта(ТекущийОбъект);
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	ЭлектронныеПодписи.ПодписанныйОбъект = &ОбъектСсылка";
...
Функция ДоступнаРегистрация(Документ)
	...
	Если ЭтоДелопроизводитель И ДокументооборотПраваДоступа.ЕстьПравоРегистрации(Документ) Тогда
...
Функция ЗаблокированныеОбъекты()
	
	Возврат ПараметрыСеанса.ЗаблокированныеОбъекты.Получить();
	
КонецФункции

Процедура ЗаблокированныеОбъектыДобавить(ОбъектДОСсылка)
	
	ЗаблокированныеОбъекты = ЗаблокированныеОбъекты();
	ЗаблокированныеОбъекты.Добавить(ОбъектДОСсылка);
	ПараметрыСеанса.ЗаблокированныеОбъекты = Новый ХранилищеЗначения(ЗаблокированныеОбъекты);
	
КонецПроцедуры

Процедура ЗаблокированныеОбъектыУдалить(ОбъектДОСсылка)
	
	ЗаблокированныеОбъекты = ЗаблокированныеОбъекты();
	Индекс = ЗаблокированныеОбъекты.Найти(ОбъектДОСсылка);
	Если Индекс <> Неопределено Тогда
		ЗаблокированныеОбъекты.Удалить(Индекс);
		ПараметрыСеанса.ЗаблокированныеОбъекты = Новый ХранилищеЗначения(ЗаблокированныеОбъекты);
	КонецЕсли;
	
КонецПроцедуры

Функция ОбъектДОЗаблокирован(ОбъектДОСсылка)
	
	Возврат ЗаблокированныеОбъекты().Найти(ОбъектДОСсылка) <> Неопределено;
...
//
Функция СвойстваРавны(ОбъектXDTO1, ОбъектXDTO2, СвойствоОбъектаXDTO)
	
	Если Не ОбъектXDTO1.Установлено(СвойствоОбъектаXDTO) Или Не ОбъектXDTO2.Установлено(СвойствоОбъектаXDTO) Тогда
		// Если свойство не установлено - нет возможности сравнить свойства.
		// Оптимистично вернем Истина.
		Возврат Истина;
	КонецЕсли;
	Если НРег(СвойствоОбъектаXDTO.Имя) = "objectid"
			И (ОбъектXDTO1.objectID.id = ""
				Или ОбъектXDTO1.objectID.type = ""
				Или ОбъектXDTO2.objectID.id = ""
				Или ОбъектXDTO2.objectID.type = "") Тогда
		// Свойство objectID может быть не задано.
		// Оптимистично вернем Истина.
		Возврат Истина;
	КонецЕсли;
	Если НРег(СвойствоОбъектаXDTO.Имя) = "externalobject"
			И (ОбъектXDTO1.externalObject.id = ""
				Или ОбъектXDTO1.externalObject.type = ""
				Или ОбъектXDTO2.externalObject.id = ""
				Или ОбъектXDTO2.externalObject.type = "") Тогда
		// Свойство externalObject может быть не задано.
		// Оптимистично вернем Истина.
		Возврат Истина;
	КонецЕсли;
	
	ЗначениеСвойства1 = ОбъектXDTO1[СвойствоОбъектаXDTO.Имя];
	ЗначениеСвойства2 = ОбъектXDTO2[СвойствоОбъектаXDTO.Имя];
...
Функция ОбъектыXDTOРавны(ОбъектXDTO1, ОбъектXDTO2)
	...
	ТипXDTO1 = ОбъектXDTO1.Тип();
	ТипXDTO2 = ОбъектXDTO2.Тип();
	
	Если (ТипXDTO1.Имя <> ТипXDTO2.Имя) Или (ТипXDTO1.URIПространстваИмен <> ТипXDTO2.URIПространстваИмен) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого СвойствоОбъектаXDTO Из ТипXDTO1.Свойства Цикл
		Если Не СвойстваРавны(ОбъектXDTO1, ОбъектXDTO2, СвойствоОбъектаXDTO) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьДокументПоШаблону(Документ, ОбъектXDTO)
	...
	Если ЗаполнятьПоШаблону Тогда
		РеквизитыШаблона = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ.Шаблон, "НоменклатураДел, ФормаДокумента");
		Если ЗначениеЗаполнено(РеквизитыШаблона.НоменклатураДел) Тогда
			Документ.НоменклатураДел = РеквизитыШаблона.НоменклатураДел;
		КонецЕсли;
		Если ЗначениеЗаполнено(РеквизитыШаблона.ФормаДокумента) Тогда
			Документ.ФормаДокумента = РеквизитыШаблона.ФормаДокумента;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ЗаполнятьПоШаблону;
