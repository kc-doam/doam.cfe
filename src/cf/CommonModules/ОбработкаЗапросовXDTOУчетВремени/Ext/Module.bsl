...
//
// Параметры:
//   НаборКолонок - Массив из Строка - набор колонок для отбора заполняемых реквизитов.
//
// Возвращаемое значение:
//   ОбъектXDTO - объект XDTO типа DMActualWork или DMError.
//
Функция ПолучитьНовуюЗаписьФактическихТрудозатрат(НаборКолонок) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|	И ФактическиеТрудозатраты.ДатаДобавления >= &ТекущаяДата
			|
			|УПОРЯДОЧИТЬ ПО
			|	ДатаДобавления УБЫВ");
