...
Функция ДобавитьЗадание(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("ОбластьДанных")
			И (Не ПараметрыСеанса.ОбластьДанныхИспользование
				Или ПараметрыСеанса.ОбластьДанныхЗначение <> ПараметрыЗадания.ОбластьДанных) Тогда
		
		// Смена области данных.
		РаботаВМоделиСервисаПереопределяемый.ВойтиВОбластьДанных(
			ПараметрыЗадания.ОбластьДанных,
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗадания, "ОбъектМетаданных"));
	...
	Попытка
		...
		Задания = ФоновыеЗадания.ПолучитьФоновыеЗадания(
			Новый Структура("ИмяМетода,Ключ,Состояние",
				ПараметрыЗадания.ИмяМетода, ПараметрыЗадания.Ключ, СостояниеФоновогоЗадания.Активно));
		
		Если ПараметрыЗадания.Свойство("Ключ") И ЗначениеЗаполнено(Задания) Тогда
			Возврат Неопределено; // Уже работает.
		КонецЕсли;
		
		ФоновыеЗадания.Выполнить(
			ПараметрыЗадания.ИмяМетода,
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗадания, "Параметры"),
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗадания, "Ключ"));
