...
Процедура УстановитьЗначениеИзмеренияИнтервалаФона(ИнтервалФона, Измерение, ЗначениеИзмерения) Экспорт
	...
	ИнтервалФона.ЗначенияИзмерений = Новый ФиксированноеСоответствие(СоответсвиеИзмерений);
	...
	Для Каждого Пользователь Из НастройкиОтображения.Пользователи Цикл
		...
		Пока ВозможныеИнтервалыФонаТекущегоДня.Количество() <> 0 Цикл
			...
			Для Каждого ИнтервалФона Из ИнтервалыФона Цикл
				...
				Если ИнтервалФона.ЗначенияИзмерений["Пользователь"] <> Пользователь Тогда
					Продолжить;
				КонецЕсли;
