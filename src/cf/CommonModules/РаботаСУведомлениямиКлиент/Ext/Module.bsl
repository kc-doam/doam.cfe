...
Процедура ИнициализироватьПеременныеОбщегоОбработчикаОжидания() Экспорт
	...
	ИспользоватьУведомленияПользователя = Ложь;
	Если ПараметрыРаботыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		ИспользоватьУведомленияПользователя = ПараметрыРаботыКлиента.ИспользоватьУведомленияПользователя;
	КонецЕсли;
	
	ПараметрыПриложения["Уведомления.ПроверятьУведомления"] = ИспользоватьУведомленияПользователя;
	...
	ПараметрыПриложения["Уведомления.ДатаПроверкиУведомлений"] =
		?(ИспользоватьУведомленияПользователя,
			ТекущаяДата() - 60,
			Дата(1, 1, 1));
