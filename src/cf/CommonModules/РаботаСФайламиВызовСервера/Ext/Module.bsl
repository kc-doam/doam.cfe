...
//   * ПорогБалансаСтраницСервисаРаспознавания - Число - остаток баланса страниц, при достижении которого отправляется
//                                                       уведомление пользователю, получающего информацию о проблемах.
//   * ПорогСрокаДействияТарифаСервисаРаспознавания - Число - количество дней до завершения тарифа для уведомления
//                                                            пользователя, получающего информацию о проблемах.
//
Функция ПараметрыРаспознаванияВСервисе() Экспорт
	...
	Параметры.Вставить("ЯзыкРаспознавания",
		ПолучитьЯзыкРаспознавания());
	
	Параметры.Вставить("ПользовательЗаданияРаспознавания",
		ПолучитьПользователяЗаданияРаспознавания());
...
Функция ПолучитьФайлВизуализации(Файл, ИдентификаторФормы, ТребуетсяЗаполнитьВизуализацию) Экспорт
	...
	ДанныеФайлаПромежуточные = СоздатьСклеенныйШтампИДанныеОтметок(
		ВладелецФайла, Файл, РеквФайла.ТекущаяВерсия, Расширение); // возможно старая подпись - тут же на лету создаем png
	
	Если Не ЗначениеЗаполнено(ДанныеФайлаПромежуточные.ДвоичныеДанныеШтампа)
		И Не ДанныеФайлаПромежуточные.ЕстьПоложенияПоКоординатам Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ДанныеФайла.Вставить("ДвоичныеДанныеШтампа", ДанныеФайлаПромежуточные.ДвоичныеДанныеШтампа);
	ДанныеФайла.Вставить("ЕстьПоложенияПоКоординатам", ДанныеФайлаПромежуточные.ЕстьПоложенияПоКоординатам);
	ДанныеФайла.Вставить("ДанныеОтметокПоКоординатам", ДанныеФайлаПромежуточные.ДанныеОтметокПоКоординатам);
...
Процедура ДобавитьВизуализациюЭП(ДанныеФайла, ИдентификаторФормы) 
	...
	Если Не ПолучитьИспользоватьImageMagickДляРаспознаванияPDF() Тогда
		ТекстСообщения = НСтр("ru = 'Невозможно добавить штамп визуализации ЭП: отключено использование ImageMagick в настройках. Обратитесь к администратору для установки настроек программы в консистентное состояние.'");
	...
	Если НРег(ДанныеФайла.Расширение) = "pdf" Тогда
		...
		Если Не ИспользоватьImageMagickДляРаспознаванияPDF Тогда // пока нет svg, IM нужен даже для "1С Предприятие"
			Возврат;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ФайлВизуализация) Тогда
		...
		ДанныеФайлаПромежуточные = СоздатьСклеенныйШтампИДанныеОтметок(
			ДанныеФайла.Владелец, ДанныеФайла.Ссылка, ДанныеФайла.Версия, ДанныеФайла.Расширение);
		ДанныеФайла.Вставить("ДвоичныеДанныеШтампа", ДанныеФайлаПромежуточные.ДвоичныеДанныеШтампа);
		ДанныеФайла.Вставить("ЕстьПоложенияПоКоординатам", ДанныеФайлаПромежуточные.ЕстьПоложенияПоКоординатам);
		ДанныеФайла.Вставить("ДанныеОтметокПоКоординатам", ДанныеФайлаПромежуточные.ДанныеОтметокПоКоординатам);
		
		Если НРег(ДанныеФайла.Расширение) = "doc" Тогда
			Если ДанныеФайлаПромежуточные.ДвоичныеДанныеШтампа <> Неопределено Тогда
				ДанныеФайла.Вставить("ТребуетсяСоздатьВизуализациюЭП", Истина);
			КонецЕсли;
			Возврат; // не создана визуализация, сейчас создадим - на клиенте, если есть отметка ЭП.
		КонецЕсли;
	...
	ИначеЕсли НРег(ДанныеФайла.Расширение) = "pdf" Тогда
		...
		Если СпособВставкиШтампаЭПВPDF = Перечисления.СпособыВставкиШтампаЭПВPDF.ImageMagickGhostscript Тогда
			...
			Для Каждого ПутьPng Из МассивПутейPng Цикл // по страницам.
				...
				Если Не ДанныеФайла.ЕстьПоложенияПоКоординатам Тогда // Обычное поведение - штамп по общим настройкам ДО:
					
					ИмяФайлаШтампа = ВременнаяПапкаДляРазархивирования + ПолучитьРазделительПути() + "stamp.png";
					ДанныеФайла.ДвоичныеДанныеШтампа.Записать(ИмяФайлаШтампа);
					РаботаСКартинками.УменьшитьШтамп(ИмяФайлаШтампа, ЧислоПодписей);
					
					НовыйПутьPng = РаботаСКартинками.НаложитьШтамп(
						ПутьPng, ИмяФайлаШтампа, ВременнаяПапкаДляРазархивирования, Счетчик, РасположениеШтампаЭПВPdf);
					
				Иначе // переданы штампы с положениями по координатам, актуально в частности для МЭДО:
					...
					НомерШтампа = 1;
					Для Каждого ДанныеОтметки Из ДанныеФайла.ДанныеОтметокПоКоординатам Цикл
						Если МЭДО.ЭтоЗаполненноеПоложениеИзображения(ДанныеОтметки.Положение) Тогда
							Если ДанныеОтметки.Положение.Страница = Счетчик 
								Или (ДанныеОтметки.Положение.Страница = Перечисления.СтраницаВставкиКартинки.Первая
									И Счетчик = 1)
								Или (ДанныеОтметки.Положение.Страница = Перечисления.СтраницаВставкиКартинки.Последняя
									И Счетчик = ВсегоСтраниц)
								Или ДанныеОтметки.Положение.Страница = Перечисления.СтраницаВставкиКартинки.Каждая Тогда
								ИмяФайлаШтампаПоКоординатам = ВременнаяПапкаДляРазархивирования
									+ ПолучитьРазделительПути() + СтрШаблон("stamp%1.png", НомерШтампа);
								ДанныеОтметки.ДвоичныеДанныеОтметки.Записать(ИмяФайлаШтампаПоКоординатам);
								ПутьПоложение = РаботаСЭП.НовыйПутьИПоложение(
									ИмяФайлаШтампаПоКоординатам, ДанныеОтметки.Положение);
					...
					Если ПутиСПоложениямиНаЭтойСтранице.Количество() <> 0 Тогда
						НовыйПутьPng = РаботаСКартинками.НаложитьШтампыСПоложениями(
							ПутьPng, ПутиСПоложениямиНаЭтойСтранице, ВременнаяПапкаДляРазархивирования, Счетчик);
					КонецЕсли;
		...
		Иначе // тут средствами платформы	
			...
			Если Не ДанныеФайла.ЕстьПоложенияПоКоординатам Тогда // Обычное поведение - штамп по общим настройкам ДО:
				...
				КартинкаШтампа = Новый Картинка(ДанныеФайла.ДвоичныеДанныеШтампа, Истина);
				...
				Описание.Высота = 24 * ЧислоПодписей;
				Описание.Ширина = 96;
			...
			Иначе // переданы штампы с положениями по координатам, актуально в частности для МЭДО:
				...
				Для Каждого ДанныеОтметки Из ДанныеФайла.ДанныеОтметокПоКоординатам Цикл
					...
					Описание = Новый ОписаниеОтображаемогоОбъектаPDF();
					Описание.Имя = "Пример" + Строка(НомерШтампа + СлучайноеЧислоДобавка);
					Описание.Объект = Новый Картинка(ДанныеОтметки.ДвоичныеДанныеОтметки, Истина);
					Описание.НомерСтраницы = 1;
...
Функция ПолучитьРасположениеШтампаЭПВPdf() Экспорт
	...
	Возврат ФайловыеФункцииПовтИсп.РасположениеШтампаЭПВPdf();
...
// Функция - создает штамп png, если его еще нет. В общем случае он состоит из нескольких отметок.
// Отметки с положениями по координатам в общую скленнную картинку штампа не добавляются, они будут обработаны
// отдельно, для случая pdf.
// В результате добавляет в ДанныеФайла 3 поля: 
//
// Параметры:
//  Документ - СправочникСсылка.ВходящиеДокументы, 
//  			СправочникСсылка.ИсходящиеДокументы,
//  			СправочникСсылка.ВнутренниеДокументы
//  Файл - СправочникСсылка.Файлы - Файл, визуализацию отметок подписей которого делаем
//  ТекущаяВерсия	 - СправочникСсылка.ВерсииФайлов - Версия от этого файла
//  Расширение - Строка - Расширение файла, для pdf особое поведение.
//
// Возвращаемое значение:
//  Структура:
//   * ДвоичныеДанныеШтампа - ДвоичныеДанные, Неопределено - "Обычные" отметки, склеенные вертикально.
// 	 * ДанныеОтметокПоКоординатам - Массив Из См. РаботаСЭП.НовыйДанныеОтметки - в нем и изначально отметки с координатами
// 																			  (МЭДО) и обычные отметки, координаты
// 																			  проставляются по настройкам ИБ в целом.
// 	 * ЕстьПоложенияПоКоординатам - Булево - Признак, что в предыдущем поле есть отметки по координатам, чтобы основной
// 	 									  алгоритм выводил "по-старому" или "но-новому". Показывает, что среди отметок
// 	 									  есть отметки с положениями по координатам. Это нужно, чтобы глобально не
// 	 									  менять старый алгоритм.
Функция СоздатьСклеенныйШтампИДанныеОтметок(Документ, Файл, ТекущаяВерсия, Расширение)
	
	Результат = Новый Структура();
	Результат.Вставить("ДвоичныеДанныеШтампа", Неопределено);
	Результат.Вставить("ДанныеОтметокПоКоординатам", Новый Массив());
	Результат.Вставить("ЕстьПоложенияПоКоординатам", Ложь);
	
	
	УстановитьПривилегированныйРежим(Истина);
	РасположениеШтампаИзНастроек = ПолучитьРасположениеШтампаЭПВPdf();
	...
	НомерШтампа = 0;
	МассивФайловЭПБезКоординат = Новый Массив(); // Для "Старого" алгоритма
	НашлиПоСсылкеНаФайл = Ложь;
	Для Каждого ЭП Из НаборЗаписей Цикл
		// Возможно отметка уже есть, онеа в РС СлужебныеФайлыДокументов может определяться либо по уникальному
		// идентификатору (новое поведение), либо по ссылке на файл (старое, и для не pdf):
		...
		ФайлОтметки = РегистрыСведений.СлужебныеФайлыДокументов.ПолучитьСлужебныйФайл(
			...
			Положение);
		Если Не ЗначениеЗаполнено(ФайлОтметки) И Не НашлиПоСсылкеНаФайл Тогда
			Положение = Неопределено;
			ФайлОтметки = РегистрыСведений.СлужебныеФайлыДокументов.ПолучитьСлужебныйФайл(
				Документ,
				Файл,
			...
			КонецЕсли;
		КонецЕсли;
		// Для подписей нашей организации:
		Если Не ЗначениеЗаполнено(ФайлОтметки) Тогда
			Если НашлиПоСсылкеНаФайл Тогда
				Продолжить; // По этой подписи файл отметки не найден, но он был ранее найден по ссылке на файл,
							// пропускаем.
			ИначеЕсли Не МЭДОПереопределяемый.ЭтоДокументМЭДОСВнешнейПодписью(Документ)
				И РаботаСЭП.ЭтоПодписьНашейОрганизации(ЭП.Отпечаток) Тогда
				ОписаниеЭП = РаботаСЭП.ОписаниеЭПДляОтметки(ЭП);
				ПутьКОтметкеЭП = РаботаСЭП.СоздатьОтметкуЭП(ОписаниеЭП);
				РаботаСЭП.ЗаписатьОтметкуЭП(Документ, Файл, ПутьКОтметкеЭП);
				...
				Если ЗначениеЗаполнено(ФайлОтметки) Тогда
					НашлиПоСсылкеНаФайл = Истина;
				Иначе
					Продолжить; // Перестраховка. Этого совсем не должно происходить.
				КонецЕсли;
			Иначе
				Продолжить; // Для внешних подписей МЭДО отсутствие отметки - ненормальная ситуация, генерить свою не надо, пропускаем.
			КонецЕсли;
		КонецЕсли;
		
		
		ДвоичныеДанныеКартинки = ПолучитьДвоичныеДанныеФайла(ФайлОтметки);
		Если МЭДО.ЭтоЗаполненноеПоложениеИзображения(Положение) Тогда
			Результат.ЕстьПоложенияПоКоординатам = Истина;
			Если НРег(Расширение) <> "pdf" Тогда
				// Для не-pdf в любом случае нужно сделать скленнную отметку без координат, старое поведение,
				// это ситуация, когда в документе есть и файлы pdf и не-pdf:
				ПутьНовогоФайла = ПолучитьИмяВременногоФайла("PNG"); //@skip-check missing-temporary-file-deletion
				ДвоичныеДанныеКартинки.Записать(ПутьНовогоФайла);
				МассивФайловЭПБезКоординат.Добавить(ПутьНовогоФайла);
				// (Временные файлы удалятся по стеку ниже, в процедуре РаботаСКартинками.ЗапуститьImageMagick)
			КонецЕсли;
		Иначе
			ПутьНовогоФайла = ПолучитьИмяВременногоФайла("PNG"); //@skip-check missing-temporary-file-deletion
			ДвоичныеДанныеКартинки.Записать(ПутьНовогоФайла);
			...
			// Для картинки без заданных координат все-равно сразу зададим координаты,
			// по настройкам информационной базы в целом, "по-умолчанию":
			// (Но для отметки ЭП в РС СлужебныеФайлыДокументов координаты храниться не будут).
			Размеры = РаботаСКартинками.ВсеРазмерыКартинки(ДвоичныеДанныеКартинки);
			Ширина = 102; // фиксированная типовая отметка.
			Высота = ?(Размеры.Нулевые, 1, Окр(Размеры.ВысотаПикс / Размеры.ШиринаПикс * Ширина)); // в пропорции к ширине.
			Положение = МЭДОСтруктурыДанных.НовыйПоложениеИзображения(
				Перечисления.СтраницаВставкиКартинки.Каждая, 0, 0, Ширина, Высота);
			УстановитьКоординатыПоложенияПоОбщейНастройке(Положение, РасположениеШтампаИзНастроек, НомерШтампа);
			
			НомерШтампа = НомерШтампа + 1; // Учитываем только эти штампы, которые первоначально без "положений".
		КонецЕсли;
		
		
		ДанныеОтметки = РаботаСЭП.НовыйДанныеОтметки(ДвоичныеДанныеКартинки, Положение);
		Результат.ДанныеОтметокПоКоординатам.Добавить(ДанныеОтметки);
		
	КонецЦикла;
	
	
	// Тоже самое, но с рег.штампом нашей организации:
	Положение = Неопределено; // см. МЭДОСтруктурыДанных.НовыйПоложениеИзображения
	...
	Если ЗначениеЗаполнено(ФайлРегШтамп) Тогда
		...
		ДвоичныеДанныеКартинки = ПолучитьДвоичныеДанныеФайла(ФайлРегШтамп);
		
		Если МЭДО.ЭтоЗаполненноеПоложениеИзображения(Положение) Тогда
			Результат.ЕстьПоложенияПоКоординатам = Истина;
		Иначе
			// Заполняется по настройкам ДО в целом:
			Размеры = РаботаСКартинками.ВсеРазмерыКартинки(ДвоичныеДанныеКартинки);
			Высота = 6;
	...
	// Тоже самое, но с рег.штампом контрагента:
	Положение = Неопределено; // см. МЭДОСтруктурыДанных.НовыйПоложениеИзображения
	ФайлРегШтамп = РегистрыСведений.СлужебныеФайлыДокументов.ПолучитьСлужебныйФайл(
		Документ,
		Файл,
	...
	// Для "старого" алгоритма и для всего, кроме pdf:
	НовыйПутьКОтметкеЭП = Неопределено;
	Если МассивФайловЭПБезКоординат.Количество() > 1 Тогда
		НовыйПутьКОтметкеЭП = РаботаСКартинками.СобратьКартинки(МассивФайловЭПБезКоординат, "PNG");
	ИначеЕсли МассивФайловЭПБезКоординат.Количество() = 1 Тогда
		НовыйПутьКОтметкеЭП = МассивФайловЭПБезКоординат[0];
	КонецЕсли;
	Если ЗначениеЗаполнено(НовыйПутьКОтметкеЭП) Тогда
		Результат.ДвоичныеДанныеШтампа = Новый ДвоичныеДанные(НовыйПутьКОтметкеЭП);
...
// Процедура является частью предыдущей процедуры, в Положение уже должны быть установлены Ширина и Высота
Процедура УстановитьКоординатыПоложенияПоОбщейНастройке(Положение, РасположениеШтампаИзНастроек, НомерШтампа)
	
	Если РасположениеШтампаИзНастроек = Перечисления.МестаВставкиКартинки.ЛевыйВерхний Тогда
		Положение.Слева = 0;
		Положение.Сверху = 5 + Положение.Высота * НомерШтампа;
	ИначеЕсли РасположениеШтампаИзНастроек = Перечисления.МестаВставкиКартинки.ЛевыйНижний Тогда
		Положение.Слева = 0;
		Положение.Сверху = 297 - Положение.Высота - Положение.Высота * НомерШтампа;
	ИначеЕсли РасположениеШтампаИзНастроек = Перечисления.МестаВставкиКартинки.ПравыйВерхний Тогда
		Положение.Слева = 210 - Положение.Ширина;
		Положение.Сверху =  5 + Положение.Высота * НомерШтампа;;
	ИначеЕсли РасположениеШтампаИзНастроек = Перечисления.МестаВставкиКартинки.ПравыйНижний Тогда
		Положение.Слева = 210 - Положение.Ширина;
		Положение.Сверху = 297 - Положение.Высота - Положение.Высота * НомерШтампа;
	КонецЕсли;
