...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор = Неопределено)
	...
	Если ОткрытьФормуЭДО(ДанныеФайла) Тогда
...
Процедура ПодготовкаФайлаКСохранениюПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	БудущийПолныйПуть = ПараметрыВыполнения.Обработчик.ДополнительныеПараметры.ПутьВыбора + ИмяСРасширением;
	Если СтрДлина(БудущийПолныйПуть) > 256 Тогда 
		
		СколькоСократить = СтрДлина(БудущийПолныйПуть) - 256;
		ИтогоДлина = СтрДлина(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии) - СколькоСократить;
		
		ПолноеНаименованиеВерсииУрезанное = Лев(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии, ИтогоДлина);
		
		ИмяСРасширением = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
			ПолноеНаименованиеВерсииУрезанное, ПараметрыВыполнения.ДанныеФайла.Расширение);
...
#Если ВебКлиент Тогда
	Если ДанныеФайла.Расширение <> "mxl" Тогда
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено,
	...
	РаботаСФайламиСлужебныйКлиент.ПодписатьФайл(
		ПрисоединенныйФайл, ИдентификаторФормы, ОбработкаРезультата);
