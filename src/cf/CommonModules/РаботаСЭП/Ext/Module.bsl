...
Функция СформироватьНаборВажныхПолей(Знач ОбъектСсылка, Версия = Неопределено,
	...
	ЗаполнятьПодписантаВФайлах = Ложь;
	ОтпечатокСертификата = Неопределено;
	
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ЗаполнятьПодписантаВФайле")
		И ДополнительныеПараметры.ЗаполнятьПодписантаВФайле = Истина Тогда
		
		ЗаполнятьПодписантаВФайлах = Истина;
		ОтпечатокСертификата = ДополнительныеПараметры.Отпечаток;
	КонецЕсли;
				...
				Если ДелопроизводствоКлиентСервер.ЭтоДокумент(ПодписываемыйОбъект)
					...
					Если ДелопроизводствоКлиентСервер.ЭтоВнутреннийДокумент(ПодписываемыйОбъект) Тогда
						...
						ИначеЕсли ТипЗнч(ПараметрыВыполнения) = Тип("Структура") И ПараметрыВыполнения.Свойство("ПодписатьЭД")
							...
							ДокументПоставленВОчередь = ОбменСКонтрагентамиДОСервер.ПоставитьДокументВОчередьФормированияЭД(
