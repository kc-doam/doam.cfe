	...
	Иначе
		...
		Если ДополнительныеПараметры.Свойство("ЗаполнятьФормализованныйЭДО")
			...
			Если ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента <> Неопределено Тогда
				ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента.Сертификат = 
					Параметры.ОписаниеДанных.ВыбранныйСертификат.Ссылка;
				ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента.Доверенность = 
...
Процедура ОтобразитьРезультатПроверкиПодписей(РезультатПроверки) Экспорт
	...
	Если РезультатПроверки.ПроверенныеПодписи.Количество() = 0 Тогда
		Возврат;
	ИначеЕсли РезультатПроверки.ПроверенныеПодписи.Количество() = 1 Тогда
...
Процедура ПродлитьДействиеПодписей(Форма, УИДПодписи,
		ОбработчикПродолжения = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(УИДПодписи) Тогда
		ВызватьИсключение НСтр("ru = 'Не выбрана подпись для продления'");
	КонецЕсли;
	
	КлючиРегистраЭП = РаботаСЭПВызовСервера.КлючиРегистраПодписейПоИдентификаторамПодписей(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(УИДПодписи));
	КлючРегистра = КлючиРегистраЭП[УИДПодписи]; // см. РаботаСЭП.НовыйКлючРегистраЭлектронныхПодписей
	
	Если КлючРегистра = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдена подпись для продления'");
	КонецЕсли;
	
	Объект = КлючРегистра.ПодписанныйОбъект;
	ПорядковыйНомер = КлючРегистра.ПорядковыйНомер;
	...
	ДанныеПодписи.Вставить("ПорядковыйНомер", ПорядковыйНомер);
...
Процедура ОтобразитьРезультатПроверкиНесколькихПодписей(РезультатПроверки)
	...
	Для Каждого Элемент Из РезультатПроверки.ПроверенныеПодписи Цикл
		...
		РезультатПроверки = Элемент.Значение;
		
		Если РезультатПроверки.ПодписьВерна Тогда
			
			Действительных = Действительных + 1;
			
			Если Не РезультатПроверки.СертификатДействителен Тогда
