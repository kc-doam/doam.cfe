...
Процедура УстановитьДокументамСтатусОбменаСАрхивом(Список, СтатусОбмена) Экспорт
	...
	Если Результат = "" Тогда 		
		ТекстРезультата = СтрШаблон(
			НСтр("ru = 'Статус ""%1"" установлен у %2 %3 ""Передача дел в Архив""'"),
			СтатусОбмена,
			ВыделенныеСтроки.Количество(),
			?(ВыделенныеСтроки.Количество() = 1, "документа", "документов"));
	Иначе
		ТекстРезультата = СтрШаблон(
			НСтр("ru = 'Статус ""%1"" был установлен не у всех документов: %2%3'"),
			СтатусОбмена,
			Символы.ПС,
			Результат);
	КонецЕсли;
