...
Функция НачатьРаспознаваниеСПомощьюСервиса(ТекущаяВерсия) Экспорт
	...
	Если Не РаботаСФайламиВызовСервера.ПолучитьИспользоватьРаспознавание()
		...
		Или Не БазаАвторизированаВСервисеРаспознавания() Тогда
...
Процедура КонтрольСрокаДействияТарифаСервисаРаспознавания() Экспорт
	...
	ДатаОкончания = ЛимитСтраницРаспознавания().ДатаОкончания;
	...
	ТекстУведомления = СтрШаблон(НСтр("ru = 'До конца действия тарифа сервиса распознавания текстов осталось %1 дней.'"),
		КоличествоДней);
...
Функция РезультатРаспознавания(ИдентификаторЗадания, ИдентификаторРезультата, ГруппаЗапроса)
	...
	Иначе
		...
		КодСостояния = РезультатКоманды.ОтветСервиса.КодСостояния;
		РезультатКомандыResult = РезультатКоманды.ОтветСервиса.ДесериализованноеЗначение["result"];
		Если ТипЗнч(РезультатКомандыResult) = Тип("Соответствие")
			И РезультатКомандыResult["response_code"] = 400 Тогда
			КодСостояния = КодСостояния_400_BadRequest();
		КонецЕсли;
		
		Если КодСостояния = КодСостояния_200_ОК() Тогда
			Результат.СтрокаВозврата = "ФайлРаспознан";
			Попытка
				Результат.РаспознанныйТекст = РезультатКомандыResult;
			Исключение
				ТекстИсключения = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			КонецПопытки;
...
Процедура УведомитьОДостиженииПороговогоЗначенияБаланса()
	...
	ТекстУведомления = ?(ТекущийБаланс <= 0,
		НСтр("ru = 'Лимит страниц в сервисе распознавания текстов исчерпан.'"),
