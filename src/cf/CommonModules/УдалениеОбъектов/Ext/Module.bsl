...
Функция УдалитьОбъект(
	...
	// Подчиненные задачи по процессу
	Если ОбщегоНазначения.ЭтоБизнесПроцесс(МетаданныеУдаляемого) Тогда
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("БизнесПроцесс", УдаляемыйОбъект);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ПодчиненныеОбъекты.Добавить(Выборка.Ссылка);
		КонецЦикла;
