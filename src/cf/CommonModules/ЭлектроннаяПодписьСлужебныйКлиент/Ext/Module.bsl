...
Процедура ОбработатьОшибкуУстановкиСпискаОтзыва(Ошибка, Контекст, ОшибкаИздателя = Ложь)
	...
	Если ТипЗнч(Ошибка) = Тип(ИнформацияОбОшибке()) Тогда
		Если Контекст.ОповещениеОЗавершении = Неопределено Тогда 
			ЭлектроннаяПодписьДокументооборотКлиент.ВывестиИнформациюОбОшибке(Ошибка);
			Возврат;
		КонецЕсли;
...
Процедура УстановитьКриптопровайдерПослеПолученияДистрибутива(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Ошибка" Тогда
		ЭлектроннаяПодписьДокументооборотКлиент.ВывестиИнформациюОбОшибке(ДлительнаяОперация.ИнформацияОбОшибке);
		Возврат;
	КонецЕсли;
