...
//
Процедура ПродолжитьВыполнениеДействийПоЭДО(ПараметрыВыполнения, РезультатДействий) Экспорт
...
Функция ЗакрытьПринудительно(ЭлектронныйДокумент, КонтекстДиагностики, Комментарий = "") Экспорт
	...
	ОбменСКонтрагентамиДОСобытия.ПриПринудительномЗакрытииДокументаЭДО(ЭлектронныйДокумент);
	Возврат Результат;
КонецФункции
...
Процедура ПослеЗагрузкиСообщения(РезультатЗагрузкиСообщения, КонтекстДиагностики)
	...
	Если РезультатЗагрузкиСообщения.ЭтоНовыйДокумент Тогда
		...
		ОбменСКонтрагентамиДОСобытия.ПослеЗагрузкиСообщенияЭДОНовогоЭлектронногоДокумента(РезультатЗагрузкиСообщения);
		Возврат;
	КонецЕсли;
...
Функция НовыеДанныеПодписи() Экспорт
	...
	// Документооборот
	Данные.Вставить("ИдентификаторПодписи");
	// Документооборот
	...
	Если Не МЧДИзДокументаСоответствуетПредставителю Тогда
		...
		Если РезультатПроверкиПодписиСДаннымиДоверенности <> Неопределено Тогда
			...
			Если ДанныеДоверенностиЗаполнены Тогда 
				Если НомераМЧД.Найти(ДанныеДоверенности.НомерДоверенности) <> Неопределено
					И Результат.НомераМЧДСИННДоверителей[ДанныеДоверенности.НомерДоверенности] <> Неопределено Тогда
					НомераМЧД.Очистить();
				КонецЕсли;
