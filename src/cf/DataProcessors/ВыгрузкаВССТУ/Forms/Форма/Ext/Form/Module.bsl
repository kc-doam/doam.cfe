...
Процедура Выгрузить(Команда)
	...
	Если Результат.Успешно Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьЗавершение", ЭтотОбъект, Результат);
		...
		Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыВыгрузкиОбращений.ВФайл") Тогда
			...
			СведенияОФайле.ВремяИзмененияУниверсальное = ОбщегоНазначенияКлиент.ДатаСеанса();
			СведенияОФайле.Автор = ПользователиКлиент.ТекущийПользователь();
...
&НаСервереБезКонтекста
Функция ЗаписатьОбращениеВJSON(Обращение, ПараметрыВыгрузки)
