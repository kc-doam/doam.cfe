...
Процедура УстановитьСоответствиеФайловИЭП()
	...
	СписокФайловДерева = Файлы.ПолучитьЭлементы();
	
	Если СписокФайловДерева.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЕстьПодписи = Ложь;
	Для Каждого СтрФайл Из СписокФайловДерева Цикл
	...
	Если Не ЕстьПодписи Тогда // оптимизация
		Возврат;
	КонецЕсли;
	
	СписокФайлов = СписокФайлов(); // создаем копию.
...
Функция СписокФайлов()   
	...
	Для Каждого СтрФайл Из Файлы.ПолучитьЭлементы() Цикл  
		
		Индекс = Файлы.ПолучитьЭлементы().Индекс(СтрФайл);
		СтруктураФайла = Новый Структура("Имя, Адрес, ИндексКартинки, Импортировать, Наименование, Расширение,Отпечаток,КомуВыданСертификат,АдресСертификата,Размер,ПолноеИмяФайла,ЭтоПодпись,ПодписьВерна,Оригинал,СертификатДействителен,ТекстОшибкиПроверкиПодписи,ТекстОшибкиПроверкиСертификата");
...
Процедура ПеренестиФайлВПодпись(ФайлыСтрока, ЭПСтрока, Сертификат = Неопределено, РезультатПроверки)
	...
	ФайлыСтрокаДерева = Неопределено;
	Для Каждого СтрФайл Из Файлы.ПолучитьЭлементы() Цикл  
		
		Если СтрФайл.Адрес = ФайлыСтрока.Адрес Тогда
			
			ФайлыСтрокаДерева = СтрФайл; 
			Прервать;
	...
	Если ФайлыСтрокаДерева <> Неопределено Тогда
		
		ФайлыСтрокаЭП = ФайлыСтрокаДерева.ПолучитьЭлементы().Добавить();   
		...
		УдалитьИзФайлыДобавленные(ЭПСтрока);
...
&НаКлиенте
Процедура УдалитьИзФайлыДобавленные(ЭПСтрока)
	
	Для Каждого СтрФайл Из Файлы.ПолучитьЭлементы() Цикл  
		
		Если СтрФайл.Адрес = ЭПСтрока.Адрес Тогда
			
			Файлы.ПолучитьЭлементы().Удалить(СтрФайл); 
