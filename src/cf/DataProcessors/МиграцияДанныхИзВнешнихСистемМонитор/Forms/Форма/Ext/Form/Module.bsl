...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("МиграцияНаНовуюВерсию") Тогда
		Отказ = Истина;
		Сообщить(НСтр("ru = 'Не включена настройка "" Миграция данных в новую версию ""'"));
	КонецЕсли;	
...
Процедура НачатьНаСервере()
	...
	Если Не РаботаСЭП.ЗавершенПереходНаНовыйФорматХраненияЭП() Тогда
		ВызватьИсключение НСтр("ru = 'Не завершен переход на новый формат хранения ЭП. Необходимо дождаться выполнения отложенных обработчиков обновления'");
	КонецЕсли;
