...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НеобходимВопросОбОтключенииАвтовизуализации() Тогда
		
		ОбработчикЗавершения = Новый ОписаниеОповещения("ПослеОтветаНаВопросОбОтключенияАвтовизуализацииПриЗаписи",
			ЭтотОбъект);
		
		ЗадатьВопросОбОтключенииАвтовизуализации(ОбработчикЗавершения);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НаборКонстант.ИспользоватьImageMagickДляРаспознаванияPDF = Ложь
			И Константы.АвтовизуализацияЭП.Получить() = Истина Тогда
...
Процедура ИспользоватьImageMagickДляРаспознаванияPDFПриИзменении(Элемент)
	...
	Если НеобходимВопросОбОтключенииАвтовизуализации() Тогда
		ОбработчикЗавершения = Новый ОписаниеОповещения("УстановитьДоступностьЭлементовПоImageMagick",
			ЭтотОбъект);
		
		ЗадатьВопросОбОтключенииАвтовизуализации(ОбработчикЗавершения);
	Иначе
		УстановитьДоступностьЭлементовПоImageMagick();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьДоступностьЭлементовПоImageMagick(Результат = Неопределено,
...
#Область ОтключениеВставкиШтампаЭП

&НаСервере
Функция НеобходимВопросОбОтключенииАвтовизуализации()
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЗаданВопросООтключенииАвтовизуализацииЭП Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ВключенаАвтовизуализацияЭП = Константы.АвтовизуализацияЭП.Получить();
	Если Не НаборКонстант.ИспользоватьImageMagickДляРаспознаванияPDF И ВключенаАвтовизуализацияЭП Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаКлиенте
Функция ЗначенияВариатовОтветаВопросаООтключенииВизуализации()
	
	ВариантыОтвета = Новый Структура;
	ВариантыОтвета.Вставить("ОтключитьВизуализацию", "ОтключитьВизуализацию");
	ВариантыОтвета.Вставить("ВключитьИспользованиеImageMagick", "ВключитьИспользованиеImageMagick");
...
&НаКлиенте
Процедура ЗадатьВопросОбОтключенииАвтовизуализации(ОбработчикЗавершения = Неопределено)
	
	ТекстВопроса = НСтр("ru = 'Для автоматической вставки штампа ЭП необходимо использование ImageMagick. Отключение использования ImageMagick выключит автоматическую вставку штампа ЭП'");
	
	ВариантыОтвета = ЗначенияВариатовОтветаВопросаООтключенииВизуализации();
	
	СписокВыбора = Новый СписокЗначений();
	СписокВыбора.Добавить(ВариантыОтвета.ОтключитьВизуализацию,
		НСтр("ru = 'Отключить автоматическую вставку штампа ЭП'"));
	СписокВыбора.Добавить(ВариантыОтвета.ВключитьИспользованиеImageMagick,
		НСтр("ru = 'Включить использование ImageMagick'"));
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОбработчикЗавершения", ОбработчикЗавершения);
	
	ОбработчикОтвета = Новый ОписаниеОповещения("ОбработатьОтветНаВопросОбОтключенииАвтовизуализации",
		ЭтотОбъект, Контекст);
	
	ПоказатьВопрос(ОбработчикОтвета, ТекстВопроса, СписокВыбора, , , НСтр("ru = 'Отключение автоматической вставки штампа ЭП'"));
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьОтветНаВопросОбОтключенииАвтовизуализации(Ответ, Контекст) Экспорт
	
	Если Ответ <> Неопределено Тогда
		ЗаданВопросООтключенииАвтовизуализацииЭП = Истина;
	КонецЕсли;
	
	Если Ответ <> Неопределено
		И Ответ <> ЗначенияВариатовОтветаВопросаООтключенииВизуализации().ОтключитьВизуализацию Тогда
...
&НаКлиенте
Процедура ПослеОтветаНаВопросОбОтключенияАвтовизуализацииПриЗаписи(Результат = Неопределено,
		ДополнительныеПараметры = Неопределено) Экспорт
