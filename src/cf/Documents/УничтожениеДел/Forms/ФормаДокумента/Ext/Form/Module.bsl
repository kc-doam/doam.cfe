	...
	Если Не Объект.Проведен
		...
		Если КоличествоДокументовКУничтожению > 0 Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ПередЗаписьюПродолжениеПослеВопросаОбУничтоженииДокументов",
				ЭтотОбъект,
				ПараметрыЗаписи);
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'Проведение приведет к уничтожению %1 документов. Файлы документов будут безвозвратно удалены. Продолжить?'"),
				Формат(КоличествоДокументовКУничтожению, "ЧГ=0"));
...
Функция КоличествоДокументовКУничтожению()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ЕСТЬNULL(КОЛИЧЕСТВО(ВнутренниеДокументы.Ссылка), 0) КАК КолДок
		|ИЗ
		...
		|ГДЕ
		|	ВнутренниеДокументы.Дело В(&Дела)
		...
		|ГДЕ
		|	ИсходящиеДокументы.Дело В(&Дела)
