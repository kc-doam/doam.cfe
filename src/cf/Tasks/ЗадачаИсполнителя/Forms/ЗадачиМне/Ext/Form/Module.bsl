...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущиеИсполнители = ТекущиеИсполнители(ТекущийПользователь);
	...
	Для Каждого СписокЗадач Из ВсеСписки Цикл
		...
		СписокЗадач.Параметры.УстановитьЗначениеПараметра("ТекущиеИсполнители", ТекущиеИсполнители);
...
&НаСервереБезКонтекста
Функция ТекущиеИсполнители(ТекущийПользователь)
	...
	Пока Выборка.Следующий() Цикл
		Массив.Добавить(Выборка.РольПользователь);
	КонецЦикла;	
...
Процедура ПродолжениеВыполненияКомандыЗадачаРассмотреноПослеПроверкиНаЗанятыеФайлы(
	...
	Если ПодписыватьРезультатВыполненияЭП Тогда
		...
		Исполнитель = ЗадачиМнеТекущийИсполнитель;
		Если Не ЗначениеЗаполнено(Исполнитель) Тогда
			Исполнитель = ПользователиКлиент.ТекущийПользователь();
		КонецЕсли;
		...
		Для Каждого Предмет Из ПредметыЗадачи Цикл
			Если ТипЗнч(Предмет.Ссылка) = Тип("СправочникСсылка.ВнутренниеДокументы")
				...
				СтруктураРезолюцииДляПодписания.АвторРезолюции = Исполнитель;
				СтруктураРезолюцииДляПодписания.ВнесРезолюцию = Исполнитель;
...
Функция МожноЛиВыполнитьЗадачу(Задача, Причина)
	...
	Если ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессСсылка.Согласование") Тогда
		...
		Если ТочкаМаршрута = БизнесПроцессы.Согласование.ТочкиМаршрута.Согласовать 
			И РаботаСВизамиСогласования.УжеЕстьВизаИсполнителяЗадачи(Задача, НайденныйПредмет, НайденнаяВиза) Тогда 
	...
	Если ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессСсылка.Согласование")
		...
		Если ТочкаМаршрута = БизнесПроцессы.Согласование.ТочкиМаршрута.Согласовать
			Или ТочкаМаршрута = БизнесПроцессы.Утверждение.ТочкиМаршрута.Утвердить Тогда
