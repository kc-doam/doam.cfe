...
Процедура РасшДОАМ_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	//// DOAM 2022-10-14 {
		Пока НЕ ПодразделениеОтветственного.Ссылка.Пустая() Цикл
			Если ПодразделениеОтветственного = Пользователь.Подразделение Тогда
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		Иначе			
			Если ПустаяСтрока(ПодразделениеОтветственного.Наименование) Тогда
		    	ПараметрСообщения = "Руководителя департамента ДОАМ";
			Иначе
			...
			Если ПроверкаПросмотраКонтрагентов Тогда
				Сообщить("Данные контрагента «" + Объект.Наименование + "» можно запросить у " + ПараметрСообщения);				
			Иначе
