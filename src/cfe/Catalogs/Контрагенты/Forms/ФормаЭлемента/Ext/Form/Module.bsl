...
Процедура РасшДОАМ_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	//// DOAM 2022-11-17 {
		...
		СсылкаОПАМ = Справочники.СтруктураПредприятия.НайтиПоНаименованию("ОПАМ");
		Если ПодразделениеОтветственного.Родитель = СсылкаОВП
		И СтрНайти(НРег(ДолжностьПользователя), "руководитель подразделения")
		//ИЛИ ПодразделениеОтветственного.Родитель = СсылкаОВП И
		ИЛИ СтрНайти(НРег(ДолжностьПользователя), "менеджер проектов") Тогда
			
			Если ПодразделениеПользователя.Родитель = СсылкаОВП
			ИЛИ ПодразделениеОтветственного.Родитель = СсылкаОПАМ Тогда
				ПодразделениеПользователя = ПодразделениеОтветственного.Родитель;
			ИначеЕсли ПодразделениеОтветственного = СсылкаОПАМ Тогда
				ПодразделениеПользователя = ПодразделениеОтветственного;
			КонецЕсли;
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		//Если РольДоступна(Метаданные.Роли.АдминистраторСистемы) Тогда
			//Сообщить(Пользователь.Ссылка.Наименование + " является Администратором Системы!");
		...
		Иначе
			...
			Если СтрНайти(НРег(ДолжностьПользователя), "руководитель ")
			ИЛИ СтрНайти(НРег(ДолжностьПользователя), "менеджер ") Тогда //// 2022-11-17
