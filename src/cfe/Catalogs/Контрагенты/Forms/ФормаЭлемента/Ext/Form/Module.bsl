...
Процедура РасшДОАМ_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	//// DOAM 2024-12 {	
		...
		// ОТЛАДКА ( Закомментировать кроме - ЭтоПолноправныйПользователь Тогда )
		...
		ИначеЕсли РольДоступна("РаботаСЗадачамиСотрудниковПодразделения")
			Или ОбщегоНазначения.РежимОтладки() Тогда
			
			РезультатПроверкиПрав = ОбщегоНазначенияДОАМ.Расш_ПолучитьПравоДоступаККарточкеКонтрагента(, Объект); // 2025-05
			...
			Если ДокументооборотПраваДоступаПереопределяемый.ЭтоДелегат(ТекущийПользователь, Истина) Тогда
				...
				Для Каждого СсылкаДелегат Из СписокДелегатов Цикл
					РезультатПроверкиПрав = ОбщегоНазначенияДОАМ.Расш_ПолучитьПравоДоступаККарточкеКонтрагента(СсылкаДелегат, Объект); // 2025-05
