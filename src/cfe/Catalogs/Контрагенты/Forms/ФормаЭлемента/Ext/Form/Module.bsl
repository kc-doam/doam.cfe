...
Функция ПолучитьПравоДоступаККарточкеКонтрагента(СсылкаПользователь) ////Экспорт // 2024-12
	...
	ПодразделениеРуководителяПользователя = СведенияПользователя.Подразделение.Родитель; // Каждая "точка" блокирует таблицу
	...
	Если ПодразделениеКарточки.ПринадлежитЭлементу(ПодразделениеРуководителяПользователя) Тогда
		...
		Если ( СведенияПользователя.Подразделение.Код = СсылкаОВП.Код // 1 = ОВП
			Или ПодразделениеРуководителяПользователя.Код = СсылкаОВП.Код )
