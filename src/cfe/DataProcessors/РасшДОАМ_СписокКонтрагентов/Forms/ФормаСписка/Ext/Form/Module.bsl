...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) // 2018-03
	...
	Если Не РольДоступна(Метаданные.Роли.АдминистраторСистемы) Тогда // DOAM 2024-11
...
Функция ПолучитьОбзорКонтрагента(Контрагент) // 2018-03
	...
	//Значение = ПолучитьЗначениеДопРеквизита(Контрагент,"Подразделение");
	//Если Не Значение = Неопределено Тогда
	//	ДобавитьРеквизит(HTMLТекст, "Черный список: ", Значение);
	//КонецЕсли;
	...
	Если РольДоступна(Метаданные.Роли.ПолныеПрава) Тогда // DOAM 2024-11
		ТекстЗапроса =
		"ВЫБРАТЬ
...
Процедура ПрименитьОтборНаСервере()
	...
	//Если РольДоступна(Метаданные.Роли.ПросмотрКонтрагентовДОАМ) Тогда
...
Функция ПечатьСпискаНаСервере()
	...
	Если Не РольДоступна(Метаданные.Роли.ПросмотрКонтрагентовДОАМ) Тогда // DAOM (ИНН) 2024-11
	...
	Для Каждого текСтрока Из ТЗ Цикл
		...
		Если Не РольДоступна(Метаданные.Роли.ПросмотрКонтрагентовДОАМ) Тогда // DOAM (ИНН) 2024-11
