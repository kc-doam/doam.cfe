...
Функция ПолучитьОбзорКонтрагента(Контрагент) // 2018-03
	...
	ДобавитьРеквизитHTML(Контрагент, HTMLТекст, "РИЦ", "РИЦ: ");
		...
		Для Каждого текЗнач Из ТЗ Цикл
			...
			Иначе
				...
				Если Выборка.Количество() > 0 Тогда // 2025-08
					РезультатПроверкиПрав = ОбщегоНазначенияДОАМ.РасшДОАМ_ПолучитьПравоДоступаККарточкеКонтрагента(, Выборка.Ссылка);
				Иначе
