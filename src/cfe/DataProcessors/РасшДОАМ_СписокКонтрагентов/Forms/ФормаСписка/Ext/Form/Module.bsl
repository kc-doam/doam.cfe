...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) // 2018-03
	...
	// Скрытие элементов на Начальной странице // 2018-03
...
Процедура ОбновитьОтборыНаСервере() // ПОДМЕНЮ.ОбновитьОтборы() 1.1
	...
	Для Каждого текРек Из МассивРеквизитов Цикл
		Если текРек.Имя <> "Список" И текРек.Имя <> "Объект" Тогда // ОШИБКА 2017-08-14
...
Процедура ПрименитьОтборНаСервере()
	...
	Если РольДоступна(Метаданные.Роли.КонтрагентыДОАМ) Тогда // Тест 2024-11
...
Функция ПечатьСпискаНаСервере()
	...
	Если Не РольДоступна(Метаданные.Роли.КонтрагентыДОАМ) Тогда // DAOM (ИНН) 2024-11
	...
	Для Каждого текСтрока Из ТЗ Цикл
		...
		Если Не РольДоступна(Метаданные.Роли.КонтрагентыДОАМ) Тогда // DOAM (ИНН) 2024-11
