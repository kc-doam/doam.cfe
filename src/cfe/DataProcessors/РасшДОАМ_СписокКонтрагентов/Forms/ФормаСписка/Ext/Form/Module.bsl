...
Функция ПечатьСпискаНаСервере()
	...
	// ВЫГРУЗКА ТЕСТ #2 // 2019-09
	//ТипДопРеквизита = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Должность");
	...
	Для Каждого текСтрока Из ТЗ Цикл
		...
		// ВЫГРУЗКА  ТЕСТ #2 // 2019-09
		//Попытка
		//	ОбластьСтрокаТаблицы.Параметры.Проект = текСтрока.Ссылка.ДополнительныеРеквизиты.Найти(ТипДопРеквизита).Значение;
		//Исключение
		//	ОбластьСтрокаТаблицы.Параметры.Проект = "";
		//КонецПопытки; 
		...
		ОбластьСтрокаТаблицы.Параметры.Последняядата 	= ПолучитьПоследнююДатуВзаимодействия(текСтрока.Ссылка);
...
&НаКлиенте
Процедура ЗаполнитьКоличествоКонтрагентовНаКлиенте()
	ЗаполнитьКоличествоКонтрагентов();
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	ЭтаФорма.ПодключитьОбработчикОжидания("ЗаполнитьКоличествоКонтрагентовНаКлиенте",6);
КонецПроцедуры
