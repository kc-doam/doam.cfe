...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) // 2018-03
	...
	Если НЕ ЭтоПолноправныйПользователь Тогда // DOAM 2025-06
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Настройки["ПоказыватьОбластьОбзора"] = Неопределено Тогда
		ПоказыватьОбластьОбзора = Истина; // 2025-05
	Иначе
...
Функция ПолучитьОбзорКонтрагента(Контрагент) // 2018-03
	...
	Иначе
		...
		Если РольДоступна("ДобавлениеИзменениеКонтрагентов") Тогда // 2024-12
...
Функция ПечатьСпискаНаСервере()
	...
	ЭтоРуководительПроектов = РольДоступна("КонтрольПроектов"); // 2025-06
	...
	Если ЭтоРуководительПроектов Тогда // DAOM (ИНН) 2025-06
	...
	Для Каждого текСтрока Из ТЗ Цикл
		...
		Если НЕ текСтрока.Ссылка.ЭтоГруппа Тогда // DOAM (ГРУППА) 2024-11
			...
			Если ЭтоРуководительПроектов Тогда // DOAM (ИНН) 2025-06
