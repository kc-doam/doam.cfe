...
&НаКлиенте
Процедура ПредставлениеHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка) // 2025-05
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) // 2018-03
	...
	Если НЕ РольДоступна("ПолныеПрава") Тогда // DOAM 2024-11
...
Функция ПолучитьОбзорКонтрагента(Контрагент) // 2018-03
	...
	Если РольДоступна("ПолныеПрава") Тогда // DOAM 2024-11
		ТекстЗапроса =
		"ВЫБРАТЬ
		...
		Для Каждого текСтрока ИЗ ТаблицаВерсий Цикл
			HTMLТекст = HTMLТекст + СтрШаблон("<tr align=""center""><td>%1</td><td>%2</td><td>%3</td></tr>", текСтрока.НомерВерсии, текСтрока.ДатаВерсии, текСтрока.АвторВерсии);
		КонецЦикла;
	...
	Если ТЗ.Количество() > 0 Тогда
		...
		Для Каждого текЗнач Из ТЗ Цикл
			...
			Иначе
				...
				РезультатПроверкиПрав = ОбщегоНазначенияДОАМ.ПолучитьПравоДоступаККарточкеКонтрагента(, КонтрагентСсылка);
				...
				Если РезультатПроверкиПрав И ТЗ.Индекс(текЗнач) < ТЗ.Количество() Тогда
					...
					HTMLТекст = HTMLТекст + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						ПолучитьНавигационнуюСсылку(КонтрагентСсылка.Ссылка),
...
Функция ПечатьСпискаНаСервере()
	...
	Если НЕ РольДоступна("ПросмотрКонтрагентовДОАМ") Тогда // DAOM (ИНН) 2024-11
	...
	Для Каждого текСтрока Из ТЗ Цикл
		...
		Если НЕ текСтрока.Ссылка.ЭтоГруппа Тогда // DOAM (ГРУППА) 2024-11
			...
			Если НЕ РольДоступна("ПросмотрКонтрагентовДОАМ") Тогда // DOAM (ИНН) 2024-11
