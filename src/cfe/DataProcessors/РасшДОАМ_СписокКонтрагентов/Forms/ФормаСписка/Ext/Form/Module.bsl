...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) // 2018-03
	...
	СоздатьДополнительныеПоляСписка();	// колонки в списке, свойства которых отмечены ВыводитьНаГлавной
	СоздатьЭлементыОтбора();			// поля для отбора, свойства которых отмечены ВыполнятьПоиск
...
Процедура ПрименитьОтборНаСервере()
	...
	Если ПараметрыСеанса.ТекущийПользователь.Наименование = "Администратор" Тогда // 2021-09 Тест
...
Функция ПечатьСпискаНаСервере()
	...
	Если ПараметрыСеанса.ТекущийПользователь.Наименование = "Администратор" Тогда // ТЕСТ ИНН (Минюст) 2024-04
